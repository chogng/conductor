...
- 常驻 stdio 引擎：`conductor-rs --stdio-worker`
...
- 这个兜底在 `cli/src/dataset.rs`，因此 `open`、`previewRows`、`assessImport` 共用。
...
- `cli/src/main.rs`：命令解析、stdio 协议、Excel 转换、自动提取主逻辑、文件处理、Origin CSV 导出。
- `cli/src/dataset.rs`：CSV 读取、编码兜底、`EngineDataset`、预览元信息、按列数值缓存。
- `cli/src/converter.rs`：Excel 读取和 CSV 写出。
- `cli/src/import.rs`：转调 `assessment` crate 构造导入 assessment。
- `cli/src/detect.rs`：曲线类型、metadata、表头和形态识别。
- `cli/src/analysis.rs`：gm、SS、自动 SS fit、基础电流指标、分析批处理。
- `cli/src/rc.rs`：Rc/TLM 分析。
- `cli/src/infer.rs`：metadata 分组推断、X 值重复分段推断、正整数 metadata 解析。
...
- conductor-rs 路径查找、进程池、超时、临时目录、IPC 参数校验都在 Electron main。
- Rust CLI 不直接接触 Electron API。
...
npm run verify:conductor-rs-artifacts
npm run verify:conductor-rs-smoke
npm run verify:rust-auto-extraction
npm run verify:rust-ss-auto
npm run verify:rust-origin-export
npm run bench:phase3
...
cargo build --release -p conductor-cli --bin conductor-rs
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\lanxi\Desktop\conductor'
cargo run --release -p conductor-cli --bin conductor-rs -- --stdio-worker
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\lanxi\Desktop\conductor'
node scripts\bench-conductor-rs-preview.mjs 'C:\Users\lanxi\Desktop\293K'
```
...
```powershell
Set-Location -LiteralPath 'C:\Users\lanxi\Desktop\conductor'
node scripts\bench-conductor-rs-xls-sidecar.mjs 'C:\Users\lanxi\Desktop\293K'
```
