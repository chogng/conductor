...
def get_originpro_module(ctx: ZipContext):
    try:
        ...
        ctx.write_error(
            ...
            message=(
                "Failed to import originpro. Ensure origin-zip-worker is built with originpro."
            ),
            exc=exc,
        )
    return op
...
def connect_originpro(ctx: ZipContext, op_module, max_attempts: int):
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            ctx.write_error(
                ...
                message=f"Failed to attach Origin via originpro: {exc}",
                exc=exc,
            )
...
def main():
    ...
    try_launch_origin(ctx, str(origin_exe_path))

    op_module = get_originpro_module(ctx)
    connect_originpro(ctx, op_module, max(1, int(args.max_com_attempts)))
