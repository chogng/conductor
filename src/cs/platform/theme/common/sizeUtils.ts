 ...
 *
 * @sample `editor.fontSize` is `--conductor-editor-fontSize`.
 */
export function asCssVariableName(sizeIdent: SizeIdentifier): string {
	return `--conductor-${sizeIdent.replace(/\./g, '-')}`;
}
...
export const workbenchSizesSchemaId = 'conductor://schemas/workbench-sizes';
