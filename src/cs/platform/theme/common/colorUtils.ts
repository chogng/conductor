 ...
 * everything is prefixed with `--conductor-`.
 *
 * @sample `editorSuggestWidget.background` is `--conductor-editorSuggestWidget-background`.
 */
export function asCssVariableName(colorIdent: ColorIdentifier): string {
	return `--conductor-${colorIdent.replace(/\./g, '-')}`;
}
...
export const workbenchColorsSchemaId = 'conductor://schemas/workbench-colors';
