...
/**
 * Warning: Once passed to a list, that list takes up the responsibility of
 * disposing it.
 */
...
export abstract class CachedListVirtualDelegate<T extends object> implements IListVirtualDelegate<T> {
  private readonly cache = new WeakMap<T, number>();

  public getHeight(element: T): number {
    return this.cache.get(element) ?? this.estimateHeight(element);
  }

  protected abstract estimateHeight(element: T): number;

  public abstract getTemplateId(element: T): string;

  public setDynamicHeight(element: T, height: number): void {
    if (height > 0) {
