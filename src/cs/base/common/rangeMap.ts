import { type IRange, type IRangedGroup, Range as BaseRange } from "src/cs/base/common/range";
...
export interface IRangeMap {
	...
	paddingTop: number;
	splice(index: number, deleteCount: number, items?: readonly IRangeMapItem[]): void;
 ...
 */
export function groupIntersect(range: IRange, groups: readonly IRangedGroup[]): IRangedGroup[] {
 ...
 */
export function consolidate(groups: readonly IRangedGroup[]): IRangedGroup[] {
	const result: IRangedGroup[] = [];
...
export class RangeMap implements IRangeMap {
	...
	public splice(index: number, deleteCount: number, items: readonly IRangeMapItem[] = []): void {
