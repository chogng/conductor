export const workbenchIpcChannels = {
  desktopAutoUpdateStatusGet: "conductor:desktop:auto-update-status:get",
  desktopAutoUpdateStatusChanged: "conductor:desktop:auto-update-status:changed",
  desktopAutoUpdateCheck: "conductor:desktop:auto-update:check",
  desktopAutoUpdateCheckAndInstall: "conductor:desktop:auto-update:check-and-install",
  desktopAutoUpdateInstallDownloaded: "conductor:desktop:auto-update:install-downloaded",
  desktopAppearanceSet: "conductor:desktop:appearance:set",
  helpWindowOpen: "conductor:help-window:open",
  templatesGet: "conductor:store:templates:get",
  templatesCreate: "conductor:store:templates:create",
  templatesDelete: "conductor:store:templates:delete",
  settingsGet: "conductor:store:settings:get",
  settingsPatch: "conductor:store:settings:patch",
  persistencePathGet: "conductor:store:persistence-path:get",
  persistencePathSet: "conductor:store:persistence-path:set",
  persistencePathChoose: "conductor:store:persistence-path:choose",
  originExeGet: "conductor:origin:exe:get",
  originExeSet: "conductor:origin:exe:set",
  originExePick: "conductor:origin:exe:pick",
  originHealthCheck: "conductor:origin:health-check",
  originRunCsv: "conductor:origin:run-csv",
  originRuntimeCleanupRun: "conductor:origin:runtime-cleanup:run",
  importPrepareRust: "conductor:import:prepare-rust",
  excelConvertRust: "conductor:excel:convert-rust",
  excelReadConvertedCsv: "conductor:excel:read-converted-csv",
  analysisDemoFilesGet: "conductor:demo:files:get",
  analysisRustEngineOpen: "conductor:rust:open",
  analysisRustEnginePreviewMeta: "conductor:rust:preview-meta",
  analysisRustEnginePreviewRows: "conductor:rust:preview-rows",
  analysisRustEngineReadCell: "conductor:rust:read-cell",
  analysisRustEngineReadCells: "conductor:rust:read-cells",
  analysisRustEngineInferAutoExtraction: "conductor:rust:infer-auto-extraction",
  analysisRustEngineProcessFile: "conductor:rust:process-file",
  analysisRustEngineAnalyzeRc: "conductor:rust:analyze-rc",
  analysisRustEngineExportOriginCsv: "conductor:rust:export-origin-csv",
  analysisOriginZipSave: "conductor:origin:zip:save",
  analysisRustEngineDispose: "conductor:rust:dispose",
} as const;

export type WorkbenchIpcChannel = (typeof workbenchIpcChannels)[keyof typeof workbenchIpcChannels];
