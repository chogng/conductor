 ...
 * table model are built by the TableModel engine.
 */
...
export type ImportTableModelSeedHeuristicConfidence = "high" | "medium" | "low";

export type ImportTableModelSeedHeuristicSource =
  | "metadata"
  | "filename"
  | "hint"
  | "label"
...
export type ImportTableModelSeedHeuristicMetadata = {
  channelFuncs: string[];
...
export type ImportTableModelSeedHeuristic = {
  confidence: ImportTableModelSeedHeuristicConfidence;
  curveType: CurveKind;
  curveTypeLabel: string | null;
  needsReview: boolean;
  reasons: string[];
  xAxisRole: AxisRole | null;
  xAxisRoleSource: ImportTableModelSeedHeuristicSource;
};

export type FastImportBadgeTableModel = {
  confidence: Extract<ImportTableModelSeedHeuristicConfidence, "medium" | "low">;
...
type ImportTableModelSeedHeuristicInput = {
  fileName?: unknown;
  fileNameRole?: AxisRole | null;
  metadata?: Partial<ImportTableModelSeedHeuristicMetadata> | null;
...
type FileEvidence = {
  reason: string;
  role: AxisRole;
  source: NonNullable<ImportTableModelSeedHeuristicSource>;
  weight: number;
};

const MAX_TABLE_MODEL_SEED_REASONS = 2;
...
export const extractImportTableModelSeedMetadata = (
  rows: Array<Array<unknown> | null | undefined>,
): ImportTableModelSeedHeuristicMetadata => {
  let setupTitle = "";
  let xAxisData = "";
  let notesText = "";
  let var1Name = "";
...
export const createFastImportBadgeTableModel = ({
  fileName,
  relativePath,
  rows,
  sheetName,
}: FastImportBadgeInput): FastImportBadgeTableModel | null => {
  const sourceText = [
    fileName,
    relativePath,
    sheetName,
...
const detectCapacitanceCurveKind = ({
  ...
  ImportTableModelSeedHeuristicInput,
...
const detectPulseVoltageCurveKind = ({
  fileName,
  metadata,
}: Pick<ImportTableModelSeedHeuristicInput, "fileName" | "metadata">): {
  confidence: ImportTableModelSeedHeuristicConfidence;
  curveType: "pv";
  reason: string;
  source: NonNullable<ImportTableModelSeedHeuristicSource>;
...
const reasonPrefixBySource: Record<NonNullable<ImportTableModelSeedHeuristicSource>, string> = {
  filename: "Filename",
  label: "Axis label",
  metadata: "Metadata",
  shape: "Shape",
...
const collectRoleEvidence = ({
  ...
  xAxisLabelHint,
  xAxisLabel,
}: ImportTableModelSeedHeuristicInput): FileEvidence[] => {
...
): NonNullable<ImportTableModelSeedHeuristicSource> | null => {
...
export const createImportTableModelSeedHeuristic = ({
  fileName,
  fileNameRole = null,
  metadata,
  xAxisLabelHint,
  xAxisLabel,
}: ImportTableModelSeedHeuristicInput): ImportTableModelSeedHeuristic => {
  const normalizedMetadata = metadata ?? {};
    ...
    const reasons = strongMetadataConflict
      ? [
          ...
          ...evidence
            ...
            .slice(0, MAX_TABLE_MODEL_SEED_REASONS)
  ...
  let confidence: ImportTableModelSeedHeuristicConfidence = "low";
