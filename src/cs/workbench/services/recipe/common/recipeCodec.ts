...
const validateSourceHintPredicate = (
  ...
  if (Array.isArray(predicate.instrumentAny) && predicate.instrumentAny.length) {
    diagnostics.push(createRecipeDiagnostic(recipeId, "recipe.unsupportedSourceHintInstrument", "Recipe sourceHint.instrumentAny is unsupported until table model provide instrument metadata."));
  }
};
