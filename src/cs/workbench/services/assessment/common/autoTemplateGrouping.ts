...
const inferRepeatedXGroupLengthFromValues = ({
  totalDataRows,
  xValues,
}: {
...
const inferRepeatedXGroupLength = ({
  ...
  return inferRepeatedXGroupLengthFromValues({
    totalDataRows,
    xValues,
  });
};
...
export const resolveAutoGroupShapeFromXValues = ({
  dataStartRowIndex,
  totalRowCount,
  xValues,
}: {
  ...
  const totalDataRows =
    Number.isInteger(totalRows) && totalRows > dataStartRowIndex
      ? totalRows - dataStartRowIndex
      : xValues.length;
  const groupSize = inferRepeatedXGroupLengthFromValues({
    totalDataRows,
    xValues,
  });
