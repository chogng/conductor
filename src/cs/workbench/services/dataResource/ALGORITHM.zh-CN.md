...
`dataRange` 的主定义来自 numeric core 和被接受的 X row span：先证明某些列在同一行区间内形成可读的连续数值块，再证明其中存在可作为 sweep / X 的列，最后用与这个 X row span 对齐的 dependent columns 形成 binding。`DataName` / `DataValue` 这类 marker 只能作为边界和标题解释证据，不能单独定义 data range。
...
DataResource
  ...
  -> 生成 BindingCandidate
  -> 生成 MeasurementClassificationCandidate
...
DataResource 也不应该用某个辅助列直接宣布 data range。marker、title、metadata
引用只能收窄或解释已经存在的 numeric evidence；如果没有连续 numeric core、
没有可接受的 X candidate、没有和 X row span 对齐的 dependent values，就只能产出
ambiguous / invalid evidence，不能自动推荐。
...
一个高置信 data range 必须先有 numeric core。边界通常再由下面信号加强：
...
只有 marker 而没有 numeric core 时不能继续生成 ready 候选。只有 numeric core
但没有 marker 时仍然可以继续生成候选；headerless numeric-only 文件就是这种情况。
当文本/数字混杂、列块不连续、X pattern 不存在、title/info marker 也不存在时，应标记为 dirty / ambiguous evidence。Review 不应该把这种数据自动推荐为 ready。
...
data range 不是通过 `DataName` / `DataValue` 这类辅助列确定的。主路径应该是：
...
numeric runs by column
  ...
  -> BindingCandidate
  -> dataRange = accepted binding 的 row/column envelope
```

具体约束：

- **numeric core**：多个目标列在相同行区间内形成连续 numeric runs，numeric
  coverage 足够高，空值和文本噪声可解释；
- **X anchor**：numeric core 内至少有一个可接受的 XRangeCandidate，例如单调 /
  fixed-step / segmented sweep / repeated pattern，或者标题证据足够强且行区间连续；
- **row span**：data range 的 start/end 主要来自被接受 XRangeCandidate 的 row
  span，dependent values 只能跟随这个 span；
- **column envelope**：列范围来自围绕 X 的 DataBlockCandidate，包括和 X row
  span 对齐的 dependent columns，不包括 marker column、metadata column、空 separator；
- **boundary stop**：空行、长文本块、列结构断裂、下一个独立 numeric core、X
  pattern 中断、重复 section 边界都可以截断 data range；
- **coherence**：如果某段数字只是在 metadata 里零散出现，或者只有一列短数字，没有
  X/dependent 对齐关系，它不能成为 data range。

`DataName` / `DataValue` 的作用只是：

- 帮助识别 title row 和 data row 的边界；
- 避免 B1500 这类文件里的 metadata 数字参与同一个候选竞争；
- 给同一 data section 内的列标题提供 source weight。

它们不能单独创建 data range。即使看到了 `DataValue`，后续列也必须形成连续
numeric core，并且至少能生成 X/dependent binding candidate。反过来，headerless
numeric-only 文件完全没有 `DataName` / `DataValue`，只要 numeric core、X anchor
和 binding coherence 足够，也应该能生成 data range。
...
time / frequency / bias
  -> matching transient / frequency / generic rules 提供 rule evidence

Cgg / capacitance
  -> rule evidence: axisTendency dependent
```

这里的 `type transfer` / `type output` 是 rule evidence 的候选结果，不是最终
measurement mode。最终 mode 必须经过后面的 sweep 角色确认和 Y family 确认，
不能只靠标题词直接判定。
...
title evidence 能强判一列是否能作为某条 rule 的 X 或 Y，但不应该绕过 numeric run 直接制造切片范围。最终 row start / row end 仍然来自被选中的 XRangeCandidate。
...
- header 命中 `Vg`、`Vd`、`time`、`frequency`、`bias` 等 rule X terms；
- 上方同列 title/info cell 命中某条 rule 的 X term；
...
- header 命中 current、capacitance 等 rule Y terms；
- 上方同列 title/info cell 和绑定的 X term 共享同一条 rule；
...
### Shared-X / Repeated-XY 归一化

`xy`、`xyyyy`、`xyxyxy` 不是三套不同算法。它们应该先被归一到 binding
层：
...
X1 Y1 X2 Y2 X3 Y3
  -> 如果 X1/X2/X3 数值序列完全一致或在容差内一致，归一为 oneX-manyY
  -> Y1/Y2/Y3 的表头成为 line legend
  -> 如果 X 序列不一致，保留 manyXYpairs 或 repeated blocks
```

这里的高标准是 X 数据相同，不是表头相似。表头相似只能作为辅助证据。
例如 `c(g:g)(CV_n256_ac_des) X/Y`、`c(g:g)(CV_n350_ac_des) X/Y`
这种表头很像，但真正让它能 collapse 成 shared-X multi-Y 的，是各组 X
列的数据完全一致。对应的 Y 表头提供 line legend；括号里或变化的子串只作为
辅助语义证据。

对于原生 `X,Y1,Y2,...Yn` 文件，例如第一列是 `Vg/Vbg`，后续列标题是
`-2,-1,0,1,2`，它本来就是 shared-X multi-Y。后续 Y 表头同样是 legend，
不是用来决定 transfer/output 的主证据。

### Measurement Family / Mode 判定

Measurement classification 是 binding 之后的证据解释，不应该反过来决定
怎么切数据。推荐顺序是：

```txt
BindingCandidate
  -> 确认 X 是否是真正 sweep
  -> 用 Y family 判断 IV / CV / derived / unknown
  -> 只有 IV family 才允许判断 transfer / output
  -> 用 sweep X 判断 transfer / output
  -> 用 source weight 和 direct pair score 处理冲突
```

#### X 只在确认是 sweep 后才决定 mode

`Vg` / `Vd` 出现在文件里不够。它可能是 sweep，也可能是 bias / step /
condition。必须先看数据形态：

- sweep：在一条 curve 内连续变化，通常单调或可分段单调；
- step / bias：在每条 curve 内基本恒定，跨 curve 改变；
- condition：只描述条件，不参与曲线 X 轴；
- unknown：形态或绑定不够明确。

只有 `xRole: "sweep"` 的 X 才能决定 IV mode。`Vg` sweep 可以支持
transfer，`Vd` sweep 可以支持 output。`Vg` step 或 `Vd` step 只能作为条件
证据，不能直接决定 mode。

#### Y 先决定 family，不决定 mode

Y 的职责是证明响应类型和置信度：

- `Cgg` / `Cgs` / `Cgd` / `Cgb` / `Capacitance` / `Cp` / `Cs`：
  优先判 CV，即使 X 是 `Vg`；
- `Id` / `Ids` / `Drain Current` / `Channel Current`：
  强 IV primary current；
- `Current` / `TotalCurrent` / `CH1 Current`：
  弱 IV current，需要 source 或 proof 辅助；
- `Ig` / `Igs` / `Gate Current`：
  leakage IV，不证明 primary transfer/output；
- `Is` / `Source Current`：
  source-current IV，不证明 primary transfer/output；
- `gm` / `Transconductance`：
  transfer-derived，不是 primary transfer Y。

所以 mode 本质上由 sweep 变量决定，但必须由 Y 证明这是 IV 类响应。
如果 Y 是 capacitance，哪怕 X 是 `Vg`，也应停在 CV family，不进入
transfer/output。

#### 直接 X/Y pair 优先于辅助列

在同一个 block 里，如果存在清晰的直接 pair：
...
这个 pair 的权重应高于附近的 `gm`、`Ig`、`Is`、proof 列或文件名线索。
例如 `Vg, Id, gm` 应该先用 `Vg -> Id` 判定 primary IV transfer；`gm` 只是
derived evidence。不能让第三列 `gm` 把结果拉向 CV 或其他类型。

#### 表头匹配用连续归一化关键词，但要分来源权重

表头匹配不应该依赖分隔符拆 token。归一化后连续匹配即可：

```txt
c(g:g)(CV_n256_ac_des)
  -> cgg/capacitance family
  -> CV_n256_ac_des 作为辅助语义证据
  -> Y 表头文本作为 legend

drain TotalCurrent(IdVg_n938_des)
  -> TotalCurrent 是 weak current
  -> IdVg 是 sweep discriminator hint
  -> Y 表头文本作为 legend
```

但是连续匹配不能退化成无脑 substring。`id`、`vg`、`vd` 这类短 key
必须结合来源和角色：

```txt
axis label / bound column header
  > DataName 所属 section header
  > trace / measurement name
  > file name
```

文件名、trace name、legend 只能作为 supporting evidence，不能单独强判
transfer/output。短 key 还要避免误伤 `device_id`、`valid`、`solid` 这类无关
字符串。

#### 内置字段要按职责分层

后续内置 rules 不应该继续把一堆字段平铺到 transfer/output 的 X/Y 里。
应该把 term 责任显式化，至少区分：
...
这不是为了增加字段数量，而是为了删除不合理的重复归属。一个 term
如果只是 weak evidence，就不应该同时在 transfer/output 中以强 Y 身份出现。

#### 保守降级

这些情况应该降级为 family unknown、IV unknown mode 或 needs adjustment：

- X 名称明确，但数据形态显示它是 step / bias，不是 sweep；
- Y 只有 `Ig` / `Is` / `gm`，没有 primary current；
- 只有 `Voltage` + `Current`，没有 gate/drain 物理角色；
- 只有 CH1/CH2，缺少接线 proof；
- `IdVg` / `IdVd` 只出现在文件名或 legend；
- transfer/output 两边证据同强；
- X/Y 数据能切，但 semantic source 太弱。

电压正负、电流正负、扫描方向不应该改变 transfer/output mode。
...
- `XYXYXY` 里多个 X 列数值完全一致，应 collapse 成 shared-X multi-Y，Y
  表头作为 legend；
- `XYXYXY` 里 X 列不一致，应保留 manyXYpairs 或 repeated blocks；
- `c(g:g)(CV_n256_ac_des) X/Y` 这类连续关键词命中 CV，Y 表头作为
  legend，括号文本作为辅助证据；
- `drain TotalCurrent(IdVg_n938_des) X/Y` 这类连续关键词命中 weak current
  和 sweep discriminator；
- `Vg, Id, gm` 应按直接 `Vg -> Id` pair 判 primary transfer，`gm` 只做
  derived evidence；
- `Vg, Cgg` 应判 CV，不应判 transfer；
- `Vg` 是 step/bias 而不是 sweep 时，不应判 transfer；
- `Ig` / `Is` 不能单独证明 primary transfer/output；
- `TotalCurrent` 的置信度应弱于 `Id` / `Ids` / `Drain Current`；
