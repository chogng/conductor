...
import { getPerfNow, logPerf, } from "src/cs/workbench/common/deviceAnalysis/perf";
...
const getDomain = (domain: number[] | null | undefined): [
    number,
    number
] => {
    ...
    if (!Number.isFinite(rawStart) || !Number.isFinite(rawEnd))
        return [0, 1];
    if (rawStart === rawEnd)
        return [rawStart - 0.5, rawEnd + 0.5];
    return [Math.min(rawStart, rawEnd), Math.max(rawStart, rawEnd)];
};
const getYDomain = (domain: number[] | null | undefined): [
    number,
    number
] => {
    const rawStart = Number(domain?.[0]);
...
const buildScale = (plotRect: PlotRect, xDomain: [
    number,
    number
], yDomain: [
    number,
    ...
    return {
        ...
        yToPx: (value) => plotRect.top + (1 - (value - yMin) / ySpan) * plotRect.height,
        pxToX: (value) => xMin + ((value - plotRect.left) / plotRect.width) * xSpan,
    };
};
const getPreparedSeries = (series: DiagnosticsSeries[]): PreparedSeries[] => series.map((item, index) => {
...
const findNearestPoint = (series: PreparedSeries, xValue: number): {
    x: number;
    y: number;
} | null => {
    const { points } = series;
...
const getCurvePointAtX = (series: PreparedSeries, xValue: number): {
    x: number;
    y: number;
} | null => {
    const { points } = series;
...
const buildLinearMinorTicks = (ticks: number[] | null | undefined, minorTickCount = 1): number[] => {
...
const CanvasDiagnosticsChart = memo(function CanvasDiagnosticsChart({ ariaLabel, axisTitleFontSize = DEFAULT_AXIS_TITLE_FONT_SIZE, locatorX, referenceLines = [], rightReservedWidth = 135, series, tickLabelFontSize = DEFAULT_TICK_LABEL_FONT_SIZE, valueUnitLabel = "", xDomain, xFactor = 1, xLabelInterval, xTickDigits, xTicks, xTooltipDigits, xUnitLabel = "V", yAxisLabel, yDomain, yTicks, yTooltipMinDigits = 2, }: CanvasDiagnosticsChartProps) {
    ...
    const legendSpace = useMemo(() => Math.max(0, Number(rightReservedWidth) || 0), [rightReservedWidth]);
    ...
    const scale = useMemo(() => (plotRect ? buildScale(plotRect, resolvedXDomain, resolvedYDomain) : null), [plotRect, resolvedXDomain, resolvedYDomain]);
    const locatorMarkers = useMemo<MarkerPoint[]>(() => {
        ...
        for (const item of preparedSeries) {
            ...
            if (!point || point.y < scale.yMin || point.y > scale.yMax)
                continue;
            markers.push({
                color: item.color,
    ...
    useEffect(() => {
        ...
        visibleXTicks.forEach((tick) => {
            const x = scale.xToPx(Number(tick));
            if (x < plotRect.left - 1 || x > plotRight + 1)
                return;
            ctx.fillText(formatNumber(Number(tick) * xFactor, { digits: xTickDigits }), x, plotBottom + TICK_LABEL_OFFSET_PX);
        });
        ctx.textAlign = "right";
        ctx.textBaseline = "middle";
        for (const tick of visibleYTicks) {
            const y = scale.yToPx(Number(tick));
            if (y < plotRect.top - 1 || y > plotBottom + 1)
                continue;
            ctx.fillText(formatNumber(Number(tick), { digits: yTickDigits }), plotRect.left - TICK_LABEL_OFFSET_PX, y);
        }
    ...
    const handleMouseMove = (event: React.MouseEvent<HTMLDivElement>) => {
        const element = wrapperRef.current;
        if (!element || !plotRect || !scale || !preparedSeries.length)
            return;
        ...
        const locatorValue = toLocatorValue(locatorX);
        const locatorPx = locatorValue !== null &&
            locatorValue >= scale.xMin &&
            locatorValue <= scale.xMax
    ...
    return (jsxs("div", {
        ...
        children: [
            ...
            locatorMarkers.map((marker, index) => (jsx("div", {
            ...
            tooltip.visible && plotRect ? (jsxs("div", {
                ...
                children: [
                    ...
                    tooltip.markers.map((marker, index) => (jsx("div", {
            ...
            tooltip.visible ? (jsx("div", {
                ...
                children: jsxs("div", {
                    ...
                    children: [
                        jsxs("div", {
                            className: "flex max-w-[200px] items-center gap-2 truncate text-xs font-medium",
                            children: [
                                jsx("span", {
