...
import { DataPreviewAreaView } from "src/cs/workbench/contrib/data/DataPreviewArea";
import TemplateManagerPreviewWorkspace, {
  TemplateManagerPreviewWorkspaceView,
} from "./templatePreviewWorkspace";
import {
...
export const createTemplateManager = ({
  ...
  const createModeContent = (mode: "select" | "save"): HTMLElement => {
    ...
    const createToggleRowHelper = (labelText: string, checked: boolean, onToggle: (val: boolean) => void) => {
      ...
      lbl.className = "template_toggle_label";
  ...
  panel.append(DataPreviewArea({
    importPanel: importSessionElement ?? null,
...
export class TemplateManagerView {
  ...
  private readonly area: DataPreviewAreaView;
  ...
  constructor(props: TemplateManagerProps) {
    ...
    this.containerRef.current = this.element;

    this.left.className = getCardClassName({
      className: "template_panel_card template_config_panel",
      variant: "fill",
    });

    this.preview = new TemplateManagerPreviewWorkspaceView(this.getPreviewProps());
    this.area = new DataPreviewAreaView({
      importPanel: props.importSessionElement ?? null,
      joinTableAndTemplateCards: true,
      tablePreview: this.preview.element,
      templatePanel: this.left,
    });
    this.element.append(this.area.element);
    this.update(props);
  }
