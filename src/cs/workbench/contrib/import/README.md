...
# 这次重构的落点
`src/cs/base/browser/ui/list/list.ts` 是列表契约层，`src/cs/base/browser/ui/list/listView.ts` 是基础虚拟列表实现，负责滚动窗口、选中、聚焦、键盘导航和必要时的虚拟化，`src/cs/base/browser/ui/list/rowCache.ts` 负责行复用，`src/cs/base/browser/ui/list/splice.ts` 负责 splice 同步工具，`src/cs/base/browser/ui/list/listWidget.ts` 是它的组件入口封装。
