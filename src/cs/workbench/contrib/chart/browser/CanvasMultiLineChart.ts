...
import { padLinearDomain, padLogDomain } from "src/cs/workbench/contrib/chart/browser/chartViewModel";
import { applyAlphaToChartColor, resolveSeriesChartColor, } from "src/cs/workbench/contrib/chart/browser/chartColors";
...
const normalizeDomainTuple = (raw: ChartDomain[keyof ChartDomain] | null | undefined): [
    number,
    number
] | null => {
...
const resolvePreviewYForScale = (value: unknown, yScaleType: "linear" | "log", yLogCurrentMode: "all" | "positive" = "all"): number | null => {
    const num = Number(value);
    if (!Number.isFinite(num))
        return null;
    if (yScaleType !== "log")
        return num;
    if (yLogCurrentMode === "positive")
        return num > 0 ? num : null;
    const abs = Math.abs(num);
    return abs > 0 ? abs : null;
};
export const resolvePreviewChartYDataRange = ({ series, yScaleType, yLogCurrentMode = "all", }: Pick<CanvasMultiLineChartProps, "series" | "yScaleType" | "yLogCurrentMode">): ResolvedPreviewYDataRange => {
    ...
    for (const chartSeries of series ?? []) {
        ...
        for (let i = 0; i < pointCount; i++) {
            const yVal = resolvePreviewYForScale(yArr[i], resolvedYScaleType, yLogCurrentMode);
            if (yVal === null)
                continue;
            if (yVal < minY)
                minY = yVal;
...
export const resolvePreviewChartDomain = ({ xGroups, series, domain, yScaleType, yLogCurrentMode = "all", }: Pick<CanvasMultiLineChartProps, "xGroups" | "series" | "domain" | "yScaleType" | "yLogCurrentMode">): ResolvedPreviewDomain => {
    ...
    const resolvedXDomain = explicitXDomain ??
...
const CanvasMultiLineChart = ({ xGroups, series, domain, xScaleFactor = 1, xUnitLabel = "V", yScaleFactor = 1, yScaleType = "linear", yLogCurrentMode = "all", yUnitLabel = "", padding = DEFAULT_PADDING, title, className, }: CanvasMultiLineChartProps): React.JSX.Element => {
    ...
    const prepared = useMemo(() => {
        ...
        for (const s of seriesWithColor) {
            ...
            if (!seriesByGroup.has(gi))
                seriesByGroup.set(gi, []);
    ...
    const resolvedDomain = useMemo(() => resolvePreviewChartDomain({
        xGroups,
        series,
        domain,
        yScaleType,
    ...
    const getScale = () => {
        ...
        if (effectiveYScaleType === "log") {
            if (yMin <= 0)
                yMin = 1e-12; // Fallback for typically small currents
            if (yMax <= yMin)
                yMax = yMin * 10;
        }
        ...
        const yToPx = (y: number) => {
            if (effectiveYScaleType === "log") {
                if (y <= 0)
                    return size.height - padding.bottom; // Clamp bottom
    ...
    const drawBase = () => {
        ...
        for (const s of prepared.series) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const displayY = resolvePreviewYForScale(yVal, resolvedDomain.effectiveYScaleType, yLogCurrentMode);
                if (displayY === null) {
                    started = false;
                    previousSign = null;
                    continue;
                }
                const sign = resolvedDomain.effectiveYScaleType === "log" && yLogCurrentMode === "all"
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        for (const s of candidates) {
            const yArr = s?.y;
            if (!yArr || idx >= yArr.length)
                continue;
            const yVal = yArr[idx];
            if (!Number.isFinite(yVal))
                continue;
            const yValue = Number(yVal);
            const displayY = resolvePreviewYForScale(yValue, resolvedDomain.effectiveYScaleType, yLogCurrentMode);
            if (displayY === null)
                continue;
            const py = yToPx(displayY);
        ...
        const bestDisplayY = resolvePreviewYForScale(bestY, resolvedDomain.effectiveYScaleType, yLogCurrentMode);
        if (bestDisplayY === null)
            return;
    ...
    const yFactor = Number.isFinite(yScaleFactor) && yScaleFactor > 0 ? yScaleFactor : 1;
    const xFactor = Number.isFinite(xScaleFactor) && xScaleFactor > 0 ? xScaleFactor : 1;
    const xSuffix = typeof xUnitLabel === "string" && xUnitLabel ? ` ${xUnitLabel}` : "";
            ...
            tooltip.visible && (jsx("div", {
                ...
                children: jsxs("div", {
                    ...
                    children: [
                        ...
                        jsx("div", {
                            className: "truncate max-w-[200px] text-xs font-medium",
                            children: tooltip.seriesName
                        }),
                        jsxs("div", {
                            ...
                            children: [
                                "x=",
                                formatNumber(tooltip.xVal * xFactor),
                                xSuffix,
                                "&nbsp; y=",
