...
import { inferTickDigitsFromTicks } from "src/cs/workbench/contrib/chart/browser/chartViewModel";
import { collectCanvasLineRuns, toFiniteCanvasNumber, valueToCanvasY, } from "src/cs/workbench/contrib/chart/browser/canvasPlot";
import { getPerfNow, isPerfEnabled, logPerf, } from "src/cs/workbench/common/deviceAnalysis/perf";
import { useCanvasChartTheme } from "src/cs/workbench/contrib/chart/browser/chartCanvasTheme";
type PlotPoint = {
    x?: number;
    y?: number;
    yPositive?: number;
...
const getCachedLogChartSeriesData = (data: PlotPoint[], plotYKey: PlotYKey): PlotPoint[] => {
    ...
    const computed = data.map((point) => ({
        ...
        [LOG_CHART_SIGN_DATA_KEY]: plotYKey === SIGNED_LOG_Y_DATA_KEY
...
const getCachedLogChartSeriesList = (seriesList: PlotSeries[], plotYKey: PlotYKey): PlotSeries[] => {
...
const withYAxisUnit = (labelRaw: string | null | undefined, unitRaw: string | null | undefined): string => {
...
const stripSpecificAxisUnitSuffix = (labelRaw: string | null | undefined, unitRaw: string | null | undefined): string => {
    const label = String(labelRaw ?? "").trim();
    const unit = String(unitRaw ?? "").trim();
    if (!label || !unit)
        return label;
...
const normalizeAxisTitleOverrideForDisplay = (value: unknown, defaultEditableLabel: string): string => {
...
type CurrentBiasHoverTarget = Extract<OverlayHoverTarget, {
    kind: "currentBias";
}>;
type SsHoverTarget = Extract<OverlayHoverTarget, {
    kind: "ss";
}>;
...
const buildLinearMinorTicks = (ticks: number[] | null | undefined, minorTickCount: number): number[] => {
...
const filterTicksToDomain = (ticks: number[] | null | undefined, min: number, max: number): number[] => {
...
const findNearestSnapX = (rawX: number, snapXs: number[], disableSnap: boolean): number => {
    if (disableSnap || !snapXs.length)
        return rawX;
    let lo = 0;
...
const plotRectHasArea = (plotRect: PlotRect | null): plotRect is PlotRect => Boolean(plotRect && plotRect.width > 0 && plotRect.height > 0);
...
const getCanvasTooltipLookup = (data: PlotPoint[], chartYDataKey: string, plotYKey: PlotYKey): CanvasTooltipLookup => {
...
const getNearestCanvasTooltipPoint = (lookup: CanvasTooltipLookup, rawX: number): CanvasTooltipPoint | null => {
    const { points } = lookup;
    if (!points.length)
        return null;
    const pickNearest = (best: CanvasTooltipPoint | null, candidate: CanvasTooltipPoint | undefined): CanvasTooltipPoint | null => {
        if (!candidate)
            return best;
        if (!best)
            return candidate;
...
const interpolateCanvasTooltipPoint = (lookup: CanvasTooltipLookup, rawX: number): CanvasTooltipPoint | null => {
    const { points } = lookup;
    if (!points.length)
        return null;
...
const sameHoverTarget = (a: OverlayHoverTarget | null, b: OverlayHoverTarget | null): boolean => {
    if (a === b)
        return true;
    if (!a || !b)
        return false;
...
const CanvasMainPlotChart = memo(function CanvasMainPlotChart({ activeFile, chartFocusedFitLine, chartPointCount, chartSeriesList, chartYDataKey, chartYDomain, chartYTicks, curveLineWidth, curveRenderMode, currentBiasMarkers, currentBiasInteraction, effectiveYScale, focusedSeriesColor, vthFitOverlays = [], focusedSeriesId, focusedSsOverlay, highlightOverlays, interactiveSeriesXs, interactiveXDomain, isSsPlot, legendContent, legendWidth, chartMargin, curveProbeX, plotType, plotXFactor, plotXUnitLabel, plotYFactor, plotYKey, plotYUnitLabel, showGrid, showMajorTicks, showMinorTicks, minorTickCount, ssInteraction, ssOverlayStyle, tickLabelFontSize, tickLabelOffsetPx, xAxisLabel, xAxisEditableLabel, onXAxisLabelChange, xTickDigits, xTicks, xTooltipDigits, axisTitleFontSize, axisTitleGapPx, yAxisLabel, yAxisEditableLabel, onYAxisLabelChange, yAxisNearZeroEpsilon, yTickDigits, }: {
    ...
    const renderedLegendContent = useMemo(() => typeof legendContent === "function"
        ? legendContent({})
    ...
    const titleTextWidth = useCallback((label: string): number => {
        const textWidth = Math.ceil(label.length * axisTitleFontSize * 0.62);
        return Math.min(AXIS_TITLE_EDIT_MAX_WIDTH_PX, Math.max(AXIS_TITLE_EDIT_MIN_WIDTH_PX, textWidth + 18));
    }, [axisTitleFontSize]);
    const axisTitleLayout = useMemo(() => {
        if (!plotRect)
            return null;
        const plotBottom = plotRect.top + plotRect.height;
        const xTitleBottom = plotBottom +
            tickLabelOffsetPx +
            tickLabelFontSize +
            axisTitleGapPx +
            axisTitleFontSize;
        const yTitleX = plotRect.left -
            tickLabelOffsetPx -
    ...
    const beginAxisTitleEdit = useCallback((axis: "x" | "y") => {
        const canEdit = axis === "x"
            ? typeof onXAxisLabelChange === "function"
            : typeof onYAxisLabelChange === "function";
        if (!canEdit)
            return;
        setEditingAxisTitle(axis);
        setEditingAxisTitleDraft(axis === "x" ? xAxisEditableLabel : yAxisEditableLabel);
    }, [
        onXAxisLabelChange,
        onYAxisLabelChange,
        xAxisEditableLabel,
    ...
    const commitAxisTitleEdit = useCallback(() => {
        ...
        const currentLabel = editingAxisTitle === "x" ? xAxisEditableLabel : yAxisEditableLabel;
    ...
    useEffect(() => {
        ...
        if (!canvas || !plotRect || !scale || size.width <= 0 || size.height <= 0)
            return;
        ...
        const drawVerticalBand = (x1Raw: number, x2Raw: number, fill: string, opacity: number) => {
            const x1 = clamp(scale.xToPx(Math.min(x1Raw, x2Raw)), plotRect.left, plotRect.left + plotRect.width);
            const x2 = clamp(scale.xToPx(Math.max(x1Raw, x2Raw)), plotRect.left, plotRect.left + plotRect.width);
            if (x2 <= x1)
                return;
            ctx.save();
            ctx.fillStyle = fill;
        ...
        const drawVerticalLine = (xRaw: number, stroke: string, opacity = 1, lineWidth = 1.5) => {
            const x = scale.xToPx(xRaw);
            if (x < plotRect.left || x > plotRect.left + plotRect.width)
                return;
            ctx.save();
        ...
        const drawVthFitOverlays = () => {
            if (!Array.isArray(vthFitOverlays) || !vthFitOverlays.length || effectiveYScale !== "linear")
                return;
            ...
            for (const overlay of vthFitOverlays) {
                ...
                if (![vth, x1, x2, y1, y2].every(Number.isFinite))
                    continue;
                ...
                ctx.fillText(`${overlay.label} ${formatNumber(vth * plotXFactor, { digits: xTickDigits })}`, clamp(vthPx + 7, plotRect.left + 4, plotRight - 92), clamp(zeroYPx - 7, plotRect.top + 14, plotBottom - 4));
            }
            ctx.restore();
        };
        ...
        const drawGridAndAxes = () => {
            ...
            if (showGrid) {
                const plotRight = plotRect.left + plotRect.width;
                const plotBottom = plotRect.top + plotRect.height;
                const isInternalXGridLine = (x: number) => x > plotRect.left + 0.5 && x < plotRight - 0.5;
                const isInternalYGridLine = (y: number) => y > plotRect.top + 0.5 && y < plotBottom - 0.5;
                ctx.setLineDash(GRID_DASH);
                ctx.strokeStyle = chartTheme.grid;
                for (const tick of visibleXTicks) {
                    const x = scale.xToPx(tick);
                    if (!isInternalXGridLine(x))
                        continue;
                    ctx.beginPath();
                    ctx.moveTo(x, plotRect.top);
            ...
            if (showMinorTicks) {
                ...
                for (const tick of visibleXMinorTicks) {
                    const x = scale.xToPx(tick);
                    if (x < plotRect.left - 0.5 || x > plotRight + 0.5)
                        continue;
                ...
                for (const tick of visibleYMinorTicks) {
                    const y = scale.yToPx(tick);
                    if (y < plotRect.top - 0.5 || y > plotBottom + 0.5)
                        continue;
                    ctx.moveTo(plotRect.left, y);
            ...
            for (const tick of visibleXTicks) {
                ...
                ctx.fillText(formatNumber(tick * plotXFactor, { digits: xTickDigits }), x, plotBottom + tickLabelOffsetPx);
            }
            for (const tick of visibleYTicks) {
                const y = scale.yToPx(tick);
                const label = effectiveYScale !== "linear"
                    ? formatLogTickLabel(Math.pow(10, tick) * plotYFactor)
                    : formatNumber(Math.abs(tick * plotYFactor) <= yAxisNearZeroEpsilon
                        ? 0
                        : tick * plotYFactor, { digits: yTickDigits });
                ctx.fillStyle = chartTheme.textSecondary;
                ctx.textAlign = "right";
        ...
        if (isSsPlot && focusedSsOverlay) {
            drawVerticalBand(focusedSsOverlay.x1, focusedSsOverlay.x2, ssOverlayStyle.fill, ssOverlayStyle.fillOpacity);
        }
        drawGridAndAxes();
        ctx.save();
        ctx.beginPath();
        ctx.rect(plotRect.left, plotRect.top, plotRect.width, plotRect.height);
        ctx.clip();
        const drawSeriesSymbols = (data: PlotPoint[] | null | undefined, color: string, radius: number, alpha = 1) => {
        ...
        const drawSeriesLine = (data: PlotPoint[] | null | undefined, color: string, width: number, alpha = 1, dash: number[] = []) => {
        ...
        for (const overlay of highlightOverlays) {
            if (!overlay.hideStartLine) {
                drawVerticalLine(Math.min(overlay.x1, overlay.x2), overlay.stroke, overlay.strokeOpacity, overlay.strokeWidth ?? 1.5);
            }
            if (!overlay.hideEndLine) {
                drawVerticalLine(Math.max(overlay.x1, overlay.x2), overlay.stroke, overlay.strokeOpacity, overlay.strokeWidth ?? 1.5);
            }
        }
        if (isSsPlot && focusedSsOverlay) {
            drawVerticalLine(Math.min(focusedSsOverlay.x1, focusedSsOverlay.x2), ssOverlayStyle.stroke, ssOverlayStyle.strokeOpacity, 2);
            drawVerticalLine(Math.max(focusedSsOverlay.x1, focusedSsOverlay.x2), ssOverlayStyle.stroke, ssOverlayStyle.strokeOpacity, 2);
        }
        for (const marker of currentBiasMarkers) {
            drawVerticalLine(marker.x, marker.stroke, marker.strokeOpacity ?? 1, marker.strokeWidth ?? 2);
        }
        if (startedAt) {
    ...
    const handleMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
        if (!plotRect || !scale || !chartSeriesList.length)
            return;
        ...
        setTooltip({
            ...
            seriesName: "",
            visible: true,
            x: clamp(cursorX + 12, plotRect.left + 4, Math.max(plotRect.left + 4, plotRect.left + plotRect.width - 280)),
            y: clamp(my + 12, plotRect.top + 4, Math.max(plotRect.top + 4, plotRect.top + plotRect.height - 110)),
        });
    }, [
    ...
    return (jsxs("div", {
        ...
        children: [
            ...
            axisTitleLayout?.x && xAxisLabel ? (jsx("div", {
            ...
            axisTitleLayout?.y && yAxisLabel ? (jsx("div", {
                className: "absolute z-[6]",
                style: {
            ...
            renderedLegendContent ? (jsx("div", {
                className: "absolute right-0 top-0 bottom-0 z-[5] flex items-center",
                style: { width: legendWidth },
                children: renderedLegendContent
            })) : null,
            tooltip.visible &&
                ...
                children: [
                    ...
                    tooltip.entries.map((entry, index) => (jsx("div", {
                            ...
                            children: jsxs("div", {
                                ...
                                children: [
                                    ...
                                    jsx("span", {
                                        ...
                                        children: entry.seriesName
                                    }),
                                    jsx("span", {
                                        className: "font-mono text-xs",
                                        style: { color: chartTheme.tooltipMuted },
                                        children: entry.valueLabel
                                    })
                                ]
                            })
                        })))
...
const ChartInteractionOverlay = memo(function ChartInteractionOverlay({ xDomain, plotArea, interactiveSeriesXs = [], currentBiasInteraction = null, ssInteraction = null, ssOverlayStyle, }: {
    xDomain: [
        number,
        number
    ];
    ...
    const normalizedSnapXs = useMemo(() => interactiveSeriesXs
        .map((value) => Number(value))
        .filter((value) => Number.isFinite(value))
        .sort((a, b) => a - b)
        .filter((value, index, arr) => index === 0 || value !== arr[index - 1]), [interactiveSeriesXs]);
    useEffect(() => {
    ...
    const xToPixel = useCallback((x: number): number => {
        const [domainMin, domainMax] = sortedDomain;
        if (!plotRectHasArea(plotRect) || !(domainMax > domainMin))
            return 0;
    ...
    const currentBiasDisplayMarkers = useMemo(() => draft?.kind === "currentBias"
        ? draft.markers
        : currentBiasInteraction?.markers ?? [], [currentBiasInteraction?.markers, draft]);
    const ssDisplayRange = useMemo(() => (draft?.kind === "ss" ? draft.range : ssInteraction?.range ?? null), [draft, ssInteraction?.range]);
    const resolveCurrentBiasHover = useCallback((plotLocalX: number): CurrentBiasHoverTarget | null => {
        if (!plotRectHasArea(plotRect))
            return null;
        let nearest: {
            role: "ion" | "ioff";
            distance: number;
        ...
        for (const marker of currentBiasDisplayMarkers) {
            if (!marker.role || !Number.isFinite(marker.x))
                continue;
    ...
    const resolveSsHover = useCallback((plotLocalX: number, shiftKey: boolean): SsHoverTarget => {
        const baseRange = ssDisplayRange;
        const hasRange = !shiftKey &&
            isFiniteNumber(baseRange?.x1) &&
            isFiniteNumber(baseRange?.x2);
        if (!hasRange)
            return { kind: "ss", mode: "new" };
        const loPixel = xToPixel(Math.min(Number(baseRange?.x1), Number(baseRange?.x2)));
        const hiPixel = xToPixel(Math.max(Number(baseRange?.x1), Number(baseRange?.x2)));
    ...
    const deriveHoverTarget = useCallback((plotLocalX: number, shiftKey: boolean): OverlayHoverTarget | null => {
    ...
    const clientXToRawDomainX = useCallback((clientX: number, element: HTMLDivElement | null): number | null => {
        if (!element || !plotRectHasArea(plotRect))
            return null;
        const rect = element.getBoundingClientRect();
        const plotWidth = plotRect.width;
        const relativeX = clamp(clientX - rect.left, 0, plotWidth);
        const [domainMin, domainMax] = sortedDomain;
    ...
    const commitAndReset = useCallback(() => {
        ...
        if (drag.kind === "currentBias" &&
            ...
            const marker = draftValue.markers.find((item) => item.role === drag.activeRole);
    ...
    const handlePointerDown = useCallback((event: any) => {
        if (!interactiveMode || !plotRectHasArea(plotRect))
            return;
        if (interactiveMode === "currentBias") {
        ...
        const localPlotX = event.clientX - event.currentTarget.getBoundingClientRect().left;
        const baseRange = ssInteraction?.range ?? null;
        const rawX = clientXToRawDomainX(event.clientX, event.currentTarget as HTMLDivElement);
        if (rawX === null)
            return;
        ...
        const hasRange = mode !== "new" &&
            isFiniteNumber(baseRange?.x1) &&
            isFiniteNumber(baseRange?.x2);
        const initialRange = mode === "new" || !hasRange
    ...
    const handlePointerMove = useCallback((event: any) => {
        ...
        if (!drag) {
            ...
            setHoverTarget((prev) => sameHoverTarget(prev, nextHover) ? prev : nextHover);
            return;
        }
        const rawX = clientXToRawDomainX(event.clientX, event.currentTarget as HTMLDivElement);
        if (rawX === null)
        ...
        if (!Number.isFinite(snappedX))
            return;
        if (drag.kind === "currentBias") {
            const markers = draft?.kind === "currentBias"
                ? draft.markers
                : currentBiasInteraction?.markers ?? [];
            const nextMarkers = markers.map((marker) => marker.role === drag.activeRole ? { ...marker, x: snappedX } : marker);
            const nextDraft = {
                kind: "currentBias",
    ...
    const hoverCursor = useMemo(() => {
        ...
        if (hoverTarget.kind === "currentBias")
            return "ew-resize";
        if (hoverTarget.kind === "ss" && hoverTarget.mode === "move")
            return "grab";
        if (hoverTarget.kind === "ss" && hoverTarget.mode === "new")
            return "crosshair";
        return "ew-resize";
                ...
                return (jsxs(Fragment, {
                    ...
                    children: [
                        ...
                        jsx("div", {
                            ...
                            children: marker.label ?? (marker.role === "ion" ? "Ion" : "Ioff")
                        })
                    ]
                }));
            }),
            ssDisplayRange ? (jsxs(Fragment, {
                children: [
                    isSsDraftActive ? (jsx("div", {
                        className: "absolute top-0 bottom-0",
                        style: {
                            left: Math.min(xToPixel(ssDisplayRange.x1), xToPixel(ssDisplayRange.x2)),
                            width: Math.abs(xToPixel(ssDisplayRange.x2) - xToPixel(ssDisplayRange.x1)),
                            backgroundColor: ssOverlayStyle.fill,
                            opacity: ssOverlayStyle.fillOpacity,
                            pointerEvents: "none",
                        }
                    })) : null,
                    hoverTarget?.kind === "ss" && hoverTarget.mode === "move" ? (jsx("div", {
                        className: "absolute",
                        style: {
                            left: Math.min(xToPixel(ssDisplayRange.x1), xToPixel(ssDisplayRange.x2)),
                            top: 0,
                    ...
                    jsx("div", {
                        className: "absolute top-2",
                        style: {
                            left: (xToPixel(ssDisplayRange.x1) + xToPixel(ssDisplayRange.x2)) / 2,
                            transform: "translateX(-50%)",
                            padding: "2px 8px",
                            borderRadius: 999,
                            border: `1px solid ${ssOverlayStyle.stroke}`,
                            backgroundColor: hoverTarget?.kind === "ss" && hoverTarget.mode === "move"
                                ? ssOverlayStyle.stroke
                                : chartTheme.overlayBadgeBackground,
                            color: hoverTarget?.kind === "ss" && hoverTarget.mode === "move"
                                ? "#ffffff"
                                : chartTheme.overlayBadgeText,
                            fontSize: 11,
                            fontWeight: 700,
                            boxShadow: chartTheme.overlayBadgeShadow,
                            display: hoverTarget?.kind === "ss" && hoverTarget.mode === "move"
                                ? "block"
                                : "none",
                            pointerEvents: "none",
                            userSelect: "none",
                    ...
                    [
                        ...
                        const isActive = hoverTarget?.kind === "ss" && hoverTarget.mode === mode;
                        return (jsxs(Fragment, {
                            key: `ss-preview-edge-${index}`,
                            children: [
                                jsx("div", {
                                    className: "absolute top-0 bottom-0",
                                    style: {
                ...
                children: "Drag to create SS window"
            })) : null,
            hoverTarget?.kind === "ss" && hoverTarget.mode === "new" ? (jsx("div", {
                className: "absolute inset-0",
                style: {
...
const MainPlotChart = memo(function MainPlotChart({ plotType, curveLineWidth = 2, curvePlotType = 202, activeFile, seriesList, xDomain, xTicks, plotXFactor, plotXUnitLabel, xTickDigits, xTooltipDigits, curveProbeX, effectiveYScale, yDomain, yTicks, yLogCurrentMode = "all", yScaleMode, plotYFactor, plotYUnitLabel, focusedSeriesId, focusedFitLine, vthFitOverlays = [], focusedSeriesColor = getChartColor(0), highlightOverlays = [], currentBiasMarkers = [], focusedSsOverlay, ssOverlayStyle, interactiveSeriesXs = [], currentBiasInteraction = null, ssInteraction = null, showGrid = true, showMajorTicks = true, showMinorTicks = true, minorTickCount = 1, tickLabelFontSize = DEFAULT_TICK_LABEL_FONT_SIZE, axisTitleFontSize = DEFAULT_AXIS_TITLE_FONT_SIZE, originTickLabelOffset, originAxisTitleGap, legendWidth = 120, legendContent = undefined, xAxisLabelOverride, yAxisLabelOverride, onXAxisLabelChange, onYAxisLabelChange, }: MainPlotChartProps) {
    ...
    const axisTitleGapPx = useMemo(() => {
        ...
        if (originGap === null)
            return DEFAULT_AXIS_TITLE_GAP_PX;
        return Math.max(0, (originGap / 100) * axisTitleFontSize * PREVIEW_AXIS_TITLE_GAP_SCALE);
    }, [axisTitleFontSize, originAxisTitleGap]);
    const chartMargin = useMemo(() => ({
        ...DEFAULT_CHART_MARGIN,
        left: Math.max(DEFAULT_CHART_MARGIN.left, Math.ceil(AXIS_TITLE_EDGE_PADDING_PX +
            axisTitleFontSize +
            tickLabelFontSize * 3.2)),
        bottom: Math.max(DEFAULT_CHART_MARGIN.bottom, Math.ceil(AXIS_TITLE_EDGE_PADDING_PX +
            axisTitleFontSize +
    ...
    const curveRenderMode = useMemo<CurveRenderMode>(() => {
        if (Number(curvePlotType) === 201)
            return "scatter";
        if (Number(curvePlotType) === 202)
            return "lineSymbol";
        return "line";
    }, [curvePlotType]);
    ...
    const chartYDataKey = useMemo(() => (effectiveYScale === "linear" ? plotYKey : LOG_CHART_Y_DATA_KEY), [effectiveYScale, plotYKey]);
    ...
    const chartPointCount = useMemo(() => chartSeriesList.reduce((sum, series) => sum + (Array.isArray(series?.data) ? series.data.length : 0), 0), [chartSeriesList]);
    useEffect(() => {
        if (!renderStartedAt)
            return;
    ...
    const chartYTicks = useMemo<number[] | null>(() => {
        if (effectiveYScale === "linear")
            return Array.isArray(yTicks) ? yTicks : null;
    ...
    const yAxisNearZeroEpsilon = useMemo(() => {
        if (effectiveYScale !== "linear")
            return 0;
        const scaledTickStep = Array.isArray(yTicks) && yTicks.length >= 2
            ? Math.abs((Number(yTicks[1]) - Number(yTicks[0])) * plotYFactor)
            : 0;
        if (!Number.isFinite(scaledTickStep) || scaledTickStep <= 0)
            return 1e-18;
    ...
    const defaultYAxisLabel = useMemo(() => isSsPlot
        ? withYAxisUnit("|Id|", plotYUnitLabel)
        : isVthPlot
            ? VTH_Y_AXIS_TITLE
            : withYAxisUnit(activeFile?.yLabel, plotYUnitLabel), [activeFile?.yLabel, isSsPlot, isVthPlot, plotYUnitLabel]);
    const defaultXAxisLabel = useMemo(() => withYAxisUnit(activeFile?.xLabel, plotXUnitLabel), [activeFile?.xLabel, plotXUnitLabel]);
    const defaultXAxisEditableLabel = useMemo(() => {
        const baseLabel = stripSpecificAxisUnitSuffix(activeFile?.xLabel, plotXUnitLabel);
    ...
    const defaultYAxisEditableLabel = useMemo(() => {
        if (isSsPlot)
            return "|Id|";
        if (isVthPlot)
            return VTH_Y_AXIS_TITLE;
        const baseLabel = stripSpecificAxisUnitSuffix(activeFile?.yLabel, plotYUnitLabel);
    ...
    const normalizedXAxisLabelOverride = useMemo(() => normalizeAxisTitleOverrideForDisplay(xAxisLabelOverride, defaultXAxisEditableLabel), [defaultXAxisEditableLabel, xAxisLabelOverride]);
    const normalizedYAxisLabelOverride = useMemo(() => normalizeAxisTitleOverrideForDisplay(yAxisLabelOverride, defaultYAxisEditableLabel), [defaultYAxisEditableLabel, yAxisLabelOverride]);
    const xAxisLabel = useMemo(() => {
        const override = normalizedXAxisLabelOverride;
        if (override)
            return withYAxisUnit(override, plotXUnitLabel);
    ...
    const yAxisLabel = useMemo(() => {
        ...
        if (override)
            return withYAxisUnit(override, plotYUnitLabel);
    ...
    const handleXAxisLabelChange = useCallback((nextLabelRaw: string) => {
        const nextLabel = String(nextLabelRaw ?? "").trim();
        onXAxisLabelChange?.(nextLabel === defaultXAxisEditableLabel ? "" : nextLabel);
    }, [defaultXAxisEditableLabel, onXAxisLabelChange]);
    const handleYAxisLabelChange = useCallback((nextLabelRaw: string) => {
        const nextLabel = String(nextLabelRaw ?? "").trim();
        onYAxisLabelChange?.(nextLabel === defaultYAxisEditableLabel ? "" : nextLabel);
    }, [defaultYAxisEditableLabel, onYAxisLabelChange]);
    const interactiveXDomain = useMemo<[
        number,
        number
    ]>(() => xDomain, [xDomain]);
    return (jsx(CanvasMainPlotChart, {
