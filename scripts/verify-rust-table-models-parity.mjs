...
const normalizeRustTableModelSeed = (value) => ({
  ...
  child.stdout.on("data", (chunk) => {
    ...
    while (true) {
      ...
      try {
        ...
        console.warn(`[${REPORT_TAG}] invalid worker JSON: ${error.message}`);
        continue;
      }
...
const main = async () => {
  ...
  try {
    for (const fixture of fixtures) {
      ...
      const expectedTableModelSeed = fixture.expectedTableModelSeed;
      const expectedMetadata = computeTableMetadata(fixture.rows);
      const rustResult = await worker.send("prepareImport", {
        fileName: fixture.name,
        path: filePath,
      });
      const rustTableModelSeed = normalizeRustTableModelSeed(
        rustResult.tableModelSeed,
      );
      const rustMetadata = {
      ...
      const tableModelSeedMismatches = compareRecords(
        expectedTableModelSeed,
        rustTableModelSeed,
        STRICT_TABLE_MODEL_SEED_FIELDS,
      );
      ...
      const summary = {
        tableModelSeedMismatches,
        file: fixture.name,
        metadataMismatches,
        passed: tableModelSeedMismatches.length === 0 && metadataMismatches.length === 0,
        rustTableModelSeed,
        rustMetadata,
        expectedTableModelSeed,
        expectedMetadata,
  ...
  if (failures.length > 0) {
    for (const failure of failures) {
      console.error(`[${REPORT_TAG}] FAIL ${failure.file}`);
      for (const mismatch of [...failure.tableModelSeedMismatches, ...failure.metadataMismatches]) {
