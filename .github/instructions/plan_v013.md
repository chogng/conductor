# plan_v013 — 完整版：Files / TableModel + Recipe/UserTemplate / Template / ReviewDecision / Explorer / SliceRequest / Slice 架构方案
...
> 核心原则：
>
> 1. Files 提供 raw table 原始模型。
> 2. Template = TableModel + Recipe/UserTemplate：Template 职责面负责把具体表格模型和固定规则/用户模板 materialize 成候选 Template。
> 3. TableModel 是正式表格模型层；旧迁移 helper 命名不能演进成独立 evidence service。
...
> 10. 旧 table-model 迁移命名已退场，不能继续作为主链路概念或兼容壳保留。
...
Template
  = TableModel + Recipe/UserTemplate
...
`TableModel + Recipe/UserTemplate -> Template -> Review`，Slice 只消费
...
```txt
TableModel + RecipeSnapshot
  -> Template recipe materializer
  -> TemplateDraft / Template

TableModel + UserTemplateSnapshot
...
### 2.2 Template TableModel

TableModel 是 Template materialization 的表格模型输入。正式服务入口是
`ITableModelService` / `TableModelRecord`；当前实现文件位于
`services/tableModel`。不要保留旧服务、record、command 名作为兼容壳或迁移别名。
不要新建独立 evidence service；目标是把
`Recipe/UserTemplate + TableModel -> Template` 收到 Template 职责面。
目标文档、目标 API 和新代码应使用 TableModel/TableModel 命名。
...
```txt
TableModel + Recipe -> TemplateDraft / Template
```
...
Template materializer 根据 `UserTemplateSnapshot` 和 `TableModel` 产生
候选 Template。

```txt
TableModel + UserTemplateSnapshot -> TemplateDraft / Template
```
...
## 3. TableModel 退场边界与 Template ownership

TableModel 只承载 raw table / column / semantic / block model。目标不是把
历史迁移形态改名成独立 evidence domain，而是把
`Recipe/UserTemplate + TableModel -> Template` 的 ownership 收进 Template。
不要让 TableModel 输出候选排序、最终模板、或系统应用建议。

旧 table-model 迁移记录可能混了两类能力：
...
不要把带旧决策字段的旧导入/持久化记录直接当作 TableModel，因为旧 record
可能还有：
...
目标记录名是 `TableModelRecord`。读取旧持久化数据或迁移 fixture 时，
必须先剥离不属于 TableModel 的字段：

```txt
retired table-model import record
  -> TableModel.fromRecord(...)
  -> TableModelRecord-compatible model only
```
...
`Template` 是 executable spec shape，也是 `TableModel + Recipe/UserTemplate`
...
```txt
rawTablesChanged / schemaProfileChanged / tableModelEngineChanged
  -> TableModel queue
  -> table-model producer
     (ITableModelService)
...
```txt
tableModelChanged / recipeChanged / userTemplateChanged
...
P3: partial table-model descriptor
P4: full table-model descriptor
```
...
```txt
prepareImportBatch
  -> TS TableModel + Recipe/UserTemplate Template materializer
  -> processFile
```
...
### 21.7 services/tableModel

正式 TableModel 合同、helper、browser 实现位于：

```txt
src/cs/workbench/services/tableModel/
  common/
    tableModel.ts
    tableModelRecord.ts
    ...
    diagnostics.ts
    blockDetector.ts
    importTableModelSeedHeuristics.ts
    schemaProfileTableModel.ts
  browser/
    importTableModelSeed.ts
    tableModelEngine.ts
    tableModelService.ts
    tableModelQueueService.ts
    tableModel.contribution.ts
```

语义收缩为：

```txt
TableModel == raw-table structure/profile/semantic/block model
TableModel != Template materializer
TableModel != Review
TableModel != primary execution domain
```

TableModel 产出 structure、profiles、semantic candidates、blocks、
diagnostics 和 table-model signature；不得产出 TemplateDraft、ReviewedTemplate、
ReviewDecision、systemRecommended 或 SliceRequest。

长期迁移目标：

```txt
table model service contracts -> src/cs/workbench/services/tableModel/common/tableModel.ts
table model record factories/helpers -> src/cs/workbench/services/tableModel/common/*
table model browser implementation -> src/cs/workbench/services/tableModel/browser/*
canonical table-models record projection helpers -> src/cs/workbench/services/tableModel/common/tableModel.ts
...
### Phase 9 — Retired table-model fields

```txt
1. 旧 table-model import/persistence record 经 TableModel.fromRecord(...) 清洗。
2. TableModel 只产出 table model，不创建 optional review。
3. 目标记录是 TableModelRecord；不要保留旧 record 名作为迁移别名。
4. 禁止把带旧决策字段的 retired record 直接透传成 TableModel。
```
...
```txt
retired table-model import with template fields -> TableModel.fromRecord drops template fields
new table model have no review fields
```
...
8. UserTemplate 独立为 services/userTemplate + contrib/userTemplate。
9. services/template 拥有 core Template spec 和 TableModel + Recipe/UserTemplate materializers。
...
15. Retired table-model imports 通过 TableModel.fromRecord 清洗为 table model，不能让旧决策字段污染 TableModelRecord。
...
9. Retired table-model imports 用正式 record factory 清洗，并向 Template table-model/materializer ownership 迁移。
