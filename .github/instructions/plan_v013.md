...
> 3. Assessment 不是目标 domain；当前 assessment 代码只是 table facts 迁移 helper，不能演进成独立 evidence service。
...
Files
  |
  v
TableFacts (current assessment compatibility record)
...
## 3. Assessment 退场与 Template TableFacts

当前 `assessment` 名称承载 table facts 生产代码。它只能继续作为迁移期
兼容实现/兼容路径名存在；目标不是把它改名成独立 evidence
domain，而是把 `TableFacts + Recipe/UserTemplate -> Template` 的 ownership
收进 Template。不要把 Assessment 做成 Review 的翻版，也不要让
Assessment 输出候选排序、最终模板、或系统应用建议。
...
```txt
Template TableFacts / current Assessment compatibility helper:
...
不要把带旧决策字段的 legacy assessment record 直接当作 TableFacts：

```ts
export type RawTableAssessmentRecord = TableFactsRecord;
```
...
目标记录名是 `RawTableFactsRecord`。`RawTableAssessmentRecord` 只能作为
legacy compatibility alias/adapter input 出现在迁移边界；只在读取旧持久化数据
或迁移 fixture 时，把旧字段当作兼容输入剥离。必须使用 adapter：

```txt
legacyAssessmentRecord
  -> TableFacts-compatible record
...
```ts
export function extractTableFactsFromLegacyAssessment(
...
legacy assessment apply 字段。Review 根据 materialized `TemplateReview`
...
2. 旧 assessment record 映射成 table facts / optional review。
3. RawTableAssessmentRecord 只能作为 legacy compatibility 名称；目标记录是 RawTableFactsRecord。
4. 禁止把带旧决策字段的 legacy assessment record 直接透传成 TableFacts。
```
...
```txt
legacy assessment with template fields -> table-fact adapter drops template fields
...
15. Legacy Assessment 通过 adapter 拆为 table facts / optional review，不能让旧决策字段污染 RawTableFactsRecord。
...
9. Legacy Assessment 用 adapter 拆分，并向 Template table-fact/materializer ownership 迁移。
