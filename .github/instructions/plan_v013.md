# plan_v013 — 完整版：Files / Template(Table + Recipe) / UserTemplate / ReviewDecision / Explorer / SliceRequest / Slice 架构方案
...
Files
  |
  v
TableFacts (current assessment migration record)
...
```txt
TableFacts / current Assessment migration helper
...
当前 `assessment` 名称承载 table facts 生产代码。它可以继续作为迁移期
服务名，但目标不是把它改名成独立 evidence
...
```txt
Template TableFacts / current Assessment helper:
...
### Template TableFacts / current Assessment migration helper
...
rawTablesChanged / schemaProfileChanged / assessmentEngineChanged
  ...
  -> assessmentChanged / tableFactsChanged projection
```

### Template materialization

```txt
assessmentChanged / recipeChanged / userTemplateChanged
...
```txt
src/cs/workbench/services/template/common/*Materializer*
```
...
3. 禁止 RawTableAssessmentRecord = TableFactsRecord 简单 alias。
```
