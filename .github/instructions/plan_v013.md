...
```txt
legacy table-fact import record
  -> RawTableFactsRecord
...
```txt
src/cs/workbench/services/tableFacts/common/legacyTableFactsAdapter.ts
```

adapter 只允许产出 TableFacts-compatible facts 或 optional review 迁移结果；
不得重新暴露旧服务、record、command 名。
...
legacy apply 字段。Review 根据 materialized `TemplateReview`
...
### Phase 9 — Legacy table-fact adapter

```txt
1. 新增 legacyTableFactsAdapter.ts。
2. 旧 table-fact import/persistence record 映射成 table facts / optional review。
3. 目标记录是 RawTableFactsRecord；不要保留旧 record 名作为迁移别名。
4. 禁止把带旧决策字段的 legacy record 直接透传成 TableFacts。
```
...
```txt
legacy table-fact import with template fields -> table-fact adapter drops template fields
...
15. Legacy table-fact imports 通过 adapter 拆为 table facts / optional review，不能让旧决策字段污染 RawTableFactsRecord。
...
9. Legacy table-fact imports 用 adapter 拆分，并向 Template table-fact/materializer ownership 迁移。
