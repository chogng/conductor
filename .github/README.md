...
| 脚本 | 职责 | 不拥有 |
| --- | --- | --- |
| `scripts/buildRecipeBundle.mjs` | 只从 `resources/recipes/v1` 生成 `services/recipes/common/builtinRecipes.generated.ts` 和 `cli/resources/recipes.v1.json`。 | 不生成旧 `services/recipe/**` 路径，不执行 Recipe。 |
| `scripts/lib/review-process.mjs` | bench / compat 脚本的请求规划 helper：整理 prepare 结果并生成 Rust `processFile` 请求。 | 不 import `services/recipes/**`、`services/review/**` 或退役 `services/tableModel/**`，不构造 `ReviewCandidate`。 |
| `scripts/verify-rust-table-models-parity.mjs` | 用显式 fixture 期望校验 Rust `tableModelSeed` 输出。 | 不把退役 TypeScript tableModel heuristic 当作 runtime owner。 |
| `scripts/verify-rust-xls-compat.mjs` | 比较 JS/Rust XLS 转换 rows，并用 Rust prepare 对转换前后 CSV 做 seed 一致性检查。 | 不 import 退役 `tableFacts` / `tableModel` TypeScript helper。 |
