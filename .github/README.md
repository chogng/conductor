...
| --- | --- | --- | --- |
| latest review association | `ITableReviewService` / review cache | `resource + sheetId?` | UI、Explorer decoration、Slice 入口按 URI 查最新有效 review。 |
...
命名的 owner 文件。兼容期已有 `ExplorerBadgeState` 可以作为旧 UI 字段暂存，
但目标语义要收敛到 Explorer decoration。

```txt
ITableReviewService.getLatestReviewSummary(resource, sheetId?)
  -> TableReviewSummary
  -> ExplorerDecorationsProvider.provideDecorations(resource)
  -> IDecorationData / ExplorerFileEntry presentation signature
```
...
扩分支或扩业务判断。新路径必须以 `resource + sheetId?` 查询 `ITableReviewService`；
如果兼容层仍需要临时桥接到同一个 Explorer decoration UI 类型，桥接文件必须标明 migration bridge，
并以新 review / slice service 覆盖对应旧入口作为删除条件。不要保留长期双路径。
...
| `contrib/files/browser/views/explorerDecorationsProvider.ts` | 对齐上游 Explorer decoration provider。`provideDecorations(resource)` 按 resource 读取 Explorer entry 和 `ITableReviewService.getLatestReviewSummary(...)`，返回 `IDecorationData`，并触发受影响 resource repaint。 | review 计算、candidate building、scoring、Template materialization、读取 table rows。 |
...
| `contrib/files/common/explorerModel.ts` | 保留 Explorer entry/tree/presentation 类型。兼容期已有 `ExplorerBadgeState` 可继续作为 UI 状态字段，但目标语义属于 Explorer decoration。 | review cache、review validity、raw-table review 兼容逻辑。 |
| `contrib/files/browser/explorerService.ts` 或 Explorer-owned browser adapter | 订阅 `ITableReviewService.onDidChangeTableReview`，按 `resource + sheetId?` 重读最新 `TableReviewSummary`，并通知 Explorer decoration / entry presentation 更新。 | review 计算、candidate building、scoring、直接读取 legacy Session review。 |
