...
| Built-in semantic rules | 无同名上游文件；更接近内置 provider/extension contribution 的数据来源。 | `resources/rules/v1/*.json` | 按领域/格式拆分的手写 JSON semantic rules。 | 不放运行时 table/materialization 逻辑。 |
...
当前脚本落点跟随同一责任划分：`scripts/lib/review-process.mjs` 只为 bench /
