...
| --- | --- | --- | --- |
| latest review association | `IReviewService` / review cache | `resource + sheetId?` | UI、Explorer decoration、Slice 入口按 URI 查最新有效 review。 |
...
IReviewService.getLatestReviewSummary({ resource, sheetId })
  ...
  -> IDecorationData / ExplorerViewer decorationsByFileKey
```
...
它们可以在迁移期作为旧数据到 UI presentation 的 adapter 保持现状，但不能作为
URI-backed `TableReview` / Explorer decoration 的目标 owner，也不能继续为新架构扩字段、
扩分支或扩业务判断。新路径必须以 `resource + sheetId?` 查询 `IReviewService`；
旧 `ExplorerFileEntry.badgeState` / `curveTypeBadgeLabel` 兼容层已经退场，不保留长期双路径。
...
| `contrib/files/browser/views/explorerDecorationsProvider.ts` | 对齐上游 Explorer decoration provider。`provideDecorations(resource)` 按 resource 读取 Explorer entry 和 `IReviewService.getLatestReviewSummary(...)`，返回 `IDecorationData`，并触发受影响 resource repaint。 | review 计算、candidate building、scoring、Template materialization、读取 table rows。 |
...
| `contrib/files/browser/explorerViewlet.ts` / `views/explorerDecorationsProvider.ts` | 订阅 review change，按 `resource + sheetId?` 重读最新 `TableReviewSummary`，并通过 `decorationsByFileKey` 通知 Explorer row presentation 更新。 | review 计算、candidate building、scoring、直接读取 legacy Session review。 |
...
| Explorer decorations provider | 上游 `explorerDecorationsProvider.ts` / `IDecorationsProvider` / decorations service。 | `contrib/files/browser/views/explorerDecorationsProvider.ts`，必要时配套同目录 `explorerDecorations.ts`。 | 从 `IReviewService.getLatestReviewSummary(...)` 的 `TableReviewSummary` 提供 `IDecorationData`、tooltip、color 和 presentation signature。 | 不读取 Session raw-table review，不 evaluate Recipe/Template，不扩 `rawTableStatusProjection` 作为新路径。 |
...
| `review.ts` / `reviewService.ts` | 使用现有 common/browser 双层。 | `common` 放 `IReviewService` 契约，`browser` 放 `resource + sheetId` summary 查询、stale、event 和后台 review 实现。 |
