...
| review output | `reviewDecision` | ranking、置信度、问题列表、decision 和 ready 分支的 `ReviewedTemplate`；不修改 `TableModel`。 |
...
| `services/review/common/reviewModel.ts` | 暂定 Review 私有输入 evidence shape：source metadata 加 `structuredEvidence` / `matrixEvidence` 这类 content facts。当前 tabular 派生字段只能作为 structured/matrix evidence 的一种形态。 | `tableProjection` 目标 API、Table UI/model imports、Recipe DSL、Review policy/result assembly、Explorer presentation。 |
...
| `services/review/common/reviewDecision.ts` | candidate + `ReviewContext` -> confidence factors、findings 和 candidate status，并组合 context、candidate、scoring 和 decision policy，产出 `ReviewResult`、selected `ReviewedTemplate` 和 summary-ready facts。 | candidate derivation、browser scheduling/cache、file/model reads、Explorer decoration mapping、Slice policy。 |
...
| Review types / context | `editor/common/model.ts`、`editor/common/model/textModel.ts` 作为 language feature 的输入模型。 | `services/review/common/reviewModel.ts`。 | 定义 URI/content evidence、`ReviewContext`、`ReviewCandidate`、`ReviewResult`、`ReviewedTemplate`、findings/factors/decision 等纯类型；tabular 派生字段只作为 structured/matrix evidence 的一种形态。 | 不放 service implementation，不放 Session migration-ledger raw-table record。 |
...
| Review service implementation | 上游类似 browser/workbench service implementation。 | `services/review/browser/reviewService.ts`。 | 按 `resource URI` + optional `sheetId` target cache `ReviewResult`，处理 stale、订阅、后台 review，并编排 `reviewSelector` / `reviewCandidate` / `reviewDecision` 这些 common helper。内部可用 `ResourceMap` 或私有索引，但 API 名称保持 resource/target 口径。 | 不读取原始文件，不拥有 Table UI/model 内容，不把 candidate 作为独立 service/model。 |
...
| Review pipeline helpers | Review service 的纯流程阶段拆分；不是独立 service。 | `services/review/common/reviewSelector.ts`、`reviewCandidate.ts`、`reviewDecision.ts`。 | selector matching、candidate building、scoring、result/summary assembly。 | browser cache、调度、file/model reads、Explorer presentation；不使用 `engine` 命名。 |
...
| `reviewModel.ts` | 建议只放纯类型。 | 定义 URI/content evidence、`ReviewContext`、`ReviewCandidate`、`ReviewResult`、`ReviewedTemplate`、findings/factors/decision；不要混入 service implementation 或 Session migration-ledger raw-table record。 |
| `reviewSelector.ts` / `reviewCandidate.ts` / `reviewDecision.ts` | 保留为 Review 流程的纯 helper 拆分。 | `browser/reviewService.ts` 编排完整 Review workflow；这些 common helper 只承载 selector matching、candidate building、scoring 和 result assembly，不形成独立 service；不要使用 `engine` 命名。 |
