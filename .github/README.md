...
| `sourceKey` / `previewInput` 是否进入 URI-backed model contract。 | 只能作为 legacy/adapter/staging 字段；新契约以 `resource URI` 为身份，多 sheet 只增加 `sheetId` 子维度。 |
...
```mermaid
flowchart TD
  A[resource URI target / optional sheetId] --> B[ITableModelService / TableModelReference]
...
| Recipe 不消费 table rows，也不自己 evaluate selector。 | `IReviewService` 内部的 selector matching 才把 selector 应用到 review evidence。 |
...
链路倒推。目标 `TableReview` 评估的是某个 `resource URI` target 上，
...
| 项 | 目标 owner | 说明 |
| --- | --- | --- |
| table identity | `resource URI` + optional `sheetId` table sub-target | 对齐上游 resource model；不使用 `sourceKey` 或公开 synthetic key。 |
...
也不从 `Template` 本身倒推结论；它只是 `TableReviewResult` 的 Explorer presentation：
...
不是完整 `TableReviewResult` internals，也不是 `ReviewedTemplate` 本身。
`TableReviewSummary` 是 review-owned 公开摘要，用来稳定表达 Explorer、Slice 入口、
...
| 字段 | 来源 | 说明 |
| --- | --- | --- |
| `resource` / `sheetId?` | review association | 被评估的 URI-backed table identity。 |
| `state` | `TableReviewResult.decision` + lifecycle | `pending`、`stale`、`ready`、`needsAdjustment`、`invalid` 这类状态。 |
| `confidence` | selected `TableCandidateReview.confidence` | 用于展示 confirmed/tentative；不由 Explorer 重新计算。 |
| `reviewedSemanticLabel` | selected candidate / `ReviewedTemplate` metadata | Explorer decoration 显示的业务标签来源，例如 transfer/output/cv。 |
| `message` / `findingCodes` | `TableReviewFinding[]` summary | tooltip 或问题说明；不暴露完整 scoring internals。 |
| `reviewSignature` | `TableReviewResult` signature | 证明 decoration 对应哪一次 review。 |
...
`TableReviewSummary.reviewedSemanticLabel`、`confidence`、`state`、
...
`RawTableReviewRecord`、Session `rawTableId`、`rawTableReviewsByRawTableId`、
slice run view input 等都属于 legacy raw-table 兼容层。它们不能作为
URI-backed `TableReview` / Explorer decoration 的目标 owner，也不能继续为新架构扩字段、
扩分支或扩业务判断。旧 raw-table status bridge 已退场；新路径必须以
...
| TableReview service contract | 上游类似 marker / language feature service 以 URI 关联结果。 | `services/review/common/review.ts`。 | 定义 `IReviewService`、`getLatestReviewSummary({ resource, sheetId })`、`onDidChangeReviewState` 等契约。 | 不放 browser cache 实现，不写 Session。 |
| TableReview service implementation | 上游类似 browser/workbench service implementation。 | `services/review/browser/tableReviewService.ts`。 | 按 `resource URI` + optional `sheetId` target cache `TableReviewResult`，处理 stale、订阅、后台 review；首版内部私有函数完成 selector matching、candidate building、scoring。内部可用 `ResourceMap` 或私有索引，但 API 名称保持 resource/target 口径。 | 不读取原始文件，不拥有 TableModel 内容，不把 candidate 作为独立 service/model。 |
| Explorer decorations service/provider | 上游 `decorationsService.ts`、`IDecorationsService`、`IDecorationsProvider`、`explorerDecorationsProvider.ts`。 | `services/decorations/**` + `contrib/files/browser/views/explorerDecorationsProvider.ts`，必要时配套同目录 `explorerDecorations.ts`。 | service 负责 provider 注册、缓存和 change event；Explorer provider 从 `IReviewService.getLatestReviewSummary(...)` 的 `TableReviewSummary` 提供 `IDecorationData`、tooltip、color 和 presentation signature。 | 不读取 Session raw-table review，不 evaluate Recipe/Template，不恢复 raw-table status bridge。 |
...
IReviewService.reviewUriTable({ resource, sheetId })
  ...
  -> SliceService retains state/result for the URI target
...
```txt
resource URI + optional sheetId + reviewSignature/templateFingerprint
```
...
| 阶段 | 输入 | 输出 | 不拥有 |
| --- | --- | --- | --- |
| Slice request | `resource`、`sheetId?`、`ReviewedTemplate`、review signature、source/model version。 | 一个可执行的 URI slice job。 | Recipe catalog、selector matching、review scoring。 |
| Slice execution | `ReviewedTemplate` 定义的 ranges / axes / series / params。 | `SliceUriResult`。 | 修改 `TableModel`、修改 `TableReviewResult`、写 Session。 |
| Slice association | `resource URI` + optional `sheetId` target + `reviewSignature/templateFingerprint`。 | 可查询的 slice 状态和结果；公开 API 用 resource/target 命名，内部索引不外露。 | `sourceKey`、legacy raw-table identity、公开 synthetic key。 |
...
| private sheet bucket | `resultsBySheetId`、`statesBySheetId` | `sourceKey`、把 sheet key 存进 model contract |
...
退场结论：`sourceKey` 在 URI-backed tablefile/table model 目标契约中退场。后续新接口不得新增 `sourceKey` 作为身份字段；如确实需要字符串 key，只能从 `resource URI + sheetId` 派生 `TableSheetKey`，并限制在 cache/storage/view adapter 内部。
...
| `TableModelSheetSnapshot.sourceKey` | 把 `resource URI + sheetId` 派生 key 存成 sheet 核心字段。 | `sheetId` / `sheetName` / `content` / `diagnostics`。 | 需要 key 时由 `toTableSheetKey(resource, sheetId)` 派生。 |
...
| --- | --- | --- | --- |
| `services/table/common/model.ts` | `editor/common/model.ts`、`editor/common/model/textModel.ts`、`workbench/common/editor/textEditorModel.ts` | `ITableModel` / `TableModel` 对应 `ITextModel` / `TextModel` 这一层，保存内容、版本、range、selection、decorations。 | 核心身份走 `resource URI`，sheet 作为 `sheetId` 子维度；`sourceKey` / `previewInput` 退场。 |
