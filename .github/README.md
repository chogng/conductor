...
解释 selector/projection、产出候选切分的是 segmentation / evaluation owner。
...
RecipeSnapshot / UserTemplateSnapshot
  -> selector / projection over content matrix
...
flowchart TD
  ...
  D --> G
  G --> H[recipe projection over content matrix]
...
| Recipe 不把 table rows 当核心输入。 | selector / projection 应作用在 canonical content matrix；二维 rows 只是 table matrix 的一种形态。 |
...
URI + content_hash/sourceVersion
  ...
  -> SegmentCandidate / ReviewCandidate(selector/projection trace + interpretation)
  -> evaluator
...
| review candidates | `ReviewCandidate[]` / `SegmentCandidate[]` | candidate builder 已经按 Recipe selector / projection 产出临时候选；当前表格 adapter 的 candidate 内含 table-shaped interpretation 和 trace。 |
| review output | `reviewResult` | ranking、置信度、问题列表、decision 和 ready 分支的 `ReviewedTemplate`；不修改 `TableModel`。 |

`Review` 可以参考 Recipe 的 rule metadata / expectation 来评分，但不能变成第二个
...
| provider selector 判断是否适用于 `ITextModel`。 | review candidate builder 内部 selector evaluator 判断 Recipe 是否适用于 `ReviewContext.evidence`。 |
