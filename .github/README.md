...
- 资源 / URI 主链路只有一条；Table 是这条链路上的 UI 渲染 / view adapter，Review 是这条链路上的内容评估结果，不经过 Table，也不属于 Table。
...
- `tableFormatId` 只属于 tabular materialization / UI 支路：它是 resource 进入 table reader / parser /
  `ITableModel` 前的稳定格式 ID，不应上升成全局 content identity。
- Files/Explorer source/open 整体遵循上游 editor open / resource model 架构；差异只在
  source content 到 canonical representation / UI projection 的 materialization 阶段。
...
- URI-backed 派生数据也按上游 resource-model 口径关联：Review、Slice、Plot 结果由各自 service/cache 按 `resource URI` 关联；多 sheet 表格只在 URI target 上增加 `sheetId` 子维度。公开 API 应使用 `resource` / `target` / model reference 命名；内部缓存可用 `ResourceMap` 或私有索引，但不能把索引名暴露成契约，也不写回 Session migration-ledger raw-table pipeline。
- 历史兼容身份字段和预览投影字段不是目标模型契约；URI-backed 链路只暴露 resource、sheetId、content version 和模型/evidence 快照。
...
阅读口径：除明确标为“迁移现状”或“迁移参考”的段落外，本文均按目标架构规划，不以当前旧实现倒推责任边界。文中的 `ITableModel` 是同一 URI/content 链路上的表格 UI materialization；`Review` 和 `Slice` 仍属于 URI 主链路上的内容评估和执行消费方。新增通用能力时应优先使用 content / matrix / segment 口径。
...
| [Files / Explorer 打开链路](#files-explorer) | source -> resource -> open/resolve/read/model 的表格 UI 支路、Clipboard 边界、责任边界和审计点。 |
...
| [Segment / Slice 输出链路](#slice) | accepted Segment、`ReviewedTemplate` 和 URI-backed Slice 结果关联。 |
...
URI
  ...
  -> structured content snapshot / CanonicalContentMatrix
      -> tabular materialization -> ITableModel / TableWidget
      -> evidence features + RecipeSnapshot / UserTemplateSnapshot / built-in template snapshot
...
Conductor:
  URI
    ...
    -> structured content snapshot / canonical content matrix
      -> evidence features / recipes / Review -> Segment[] / ReviewedTemplate -> Slice / output renderers
      -> table UI materialization -> ITableModel / TableWidget renderer
```
...
“这个 URI 当前版本的内容如何被规范化成 canonical content matrix”；表格只是同一
URI/content 链路上的一种可渲染 UI 投影。对于 table UI materialization，Files/Table 的目标不是普通文本模型，而是
...
资源 / URI 主链路只有一条。本节只描述同一 URI/content 链路上的表格 UI
打开和渲染支路，不是 `URI -> Table -> Review -> Slice` 的主流程。Review/Slice
仍按 `resource URI + content_hash/sourceVersion` 关联内容评估和执行结果。

表格打开支路整体按上游 editor open / resource model 架构走，可以认为主形状和上游一样：
...
flowchart TD
  ...
  C --> D[SourceContent + content_hash/sourceVersion]
  D --> E[StructuredContentSnapshot / CanonicalContentMatrix]

  E --> EF[evidence features]
  EF --> R[Review candidate builder / evaluator]
  R --> S[ReviewResult / ReviewedTemplate / accepted Segment]
  S --> X[Slice / output renderers]

  C --> P[Explorer resource entry / pending state]
  P --> O[ExplorerViewPane 展示 resource entry]
  O -->|open table UI| T[tabular materialization / table UI branch]
  E --> T
  T --> F[ITableService.open resource + optional sheetId]
  ...
  J -->|需要内容采样| SA[tableFileReader.readSample / first chunk]
  SA --> DT[detector resolve tableFormatId]
  DT -->|unsupported| U
  DT -->|可 materialize| K
  ...
  N --> RC[createResolvedContent]
  RC --> TMRC[TableModelResolvedContent]
  TMRC --> TM[TableModel.applyResolvedContent]
  TM --> TMS[TableModelSnapshot]
  TMS --> VM[TableViewModel 派生 view state / cache / selection]
  VM --> W[TableWidget 渲染 rows / columns / sheets]
```
...
- Session migration-ledger raw-table pipeline 不属于目标 URI-backed file-to-table 主线。
- Review / Slice 不从 `ITableService.open` 或 `TableWidget` 进入；它们按 resource/content version 消费 canonical/structured evidence 和 reviewed template。

当前实现后续需要按这条表格打开支路逐段审计。下面是潜在偏差点，不代表已经逐项确认：
...
Conductor:
  CanonicalContentMatrix(uri + content_hash)
    -> RecipeSnapshot / UserTemplateSnapshot / built-in template snapshot
    -> SegmentCandidate[]
    -> Evaluator
    -> Segment[]
...
只有 evaluator 通过后才产出 accepted `Segment`。在当前 tabular / structured adapter 迁移里，
...
、UserTemplate 或 built-in template snapshot 套到当前 canonical content matrix 后
得到的临时 segment 解释和 trace；它不是最终 Segment，也不是独立 service/model。
...
flowchart TD
  ...
  C --> U[tabular UI materialization]
  U --> V[ITableModel / TableWidget renderer]
  F[RecipeSnapshot / UserTemplateSnapshot / built-in template] --> G[recipe layout/relation evaluation]
  D --> G
  ...
  K --> L[ReviewResult / ReviewedTemplate / output renderer / Slice]
...
| Recipe 不决定 `tableFormatId`。 | format 仍由 `TableFormatService` / detector 负责，且只服务 tabular materialization / UI 支路。 |
...
`Review` 是 URI-backed content feature 链路上的候选评估，不是代码审查，也不是
Table UI / `ITableService` 的下游。目标 `Review` 评估的是某个
`resource URI + content_hash/sourceVersion` 上的 content evidence；当前 tabular 场景只是
structured evidence 的一个 adapter，不能把 `TableWidget`、`TableViewModel`、
preview projection 或 table service contract 带进 Review 公共边界。

通用层使用 `SegmentCandidate`、`Segment`、`content_hash` 和 selector provenance；
只有当 accepted segment 需要进入当前 tabular / structured Slice adapter 时，才在 adapter 边界生成
`ReviewedTemplate` 给 Slice 消费：

```txt
URI + content_hash/sourceVersion
  -> canonical content evidence
  -> ReviewEvidence(sourceMetadata + structured content evidence)
  -> SegmentCandidate / ReviewCandidate(layout/relation trace + interpretation)
  -> evaluator
  -> ReviewResult / accepted Segment

Current tabular evidence adapter:
  URI + content version + optional sheetId
    -> structured matrix evidence (rows/cells/blocks + diagnostics + profiles)
    -> tabular ReviewCandidate
    -> ReviewedTemplate
    -> Slice
```
...
| 项 | 目标 owner | 说明 |
| --- | --- | --- |
| review association | `resource URI` + `contentHash/sourceVersion` + optional selector / `sheetId` sub-target | 对齐上游 resource model；不使用公开 synthetic key；`sheetId` 只是结构化内容的子目标。 |
| review context | `ReviewContext` | 包含 `resource`、可选 sub-target、content hash/source version、evidence fingerprint、可选 materialization version 和 canonical content evidence；不读文件、不访问 browser `File`。 |
| review candidates | `ReviewCandidate[]` / `SegmentCandidate[]` | candidate builder 已经按 Recipe dataRange / blockPartition / physicalLayout / logicalRelation 产出临时候选；当前 tabular adapter 的 candidate 内含结构化矩阵解释和 trace。 |
...
```txt
1. hard gates
   parser fatal / stale content/evidence version / missing required candidate slot / invalid range
...
| 条件 | 结果 |
| --- | --- |
| `ReviewCandidate` 的 contentHash/sourceVersion/evidenceFingerprint 已过期。 | `invalid`，confidence = 0。 |
| parser 有 fatal diagnostic，或 `ReviewContext.evidence` 不存在。 | `invalid`，confidence = 0。 |
| required evidence / selector slot 缺失。 | `needsAdjustment`，confidence cap 0.49。 |
...
| `projectionScore` | `ReviewCandidate.projectionTrace` / structured interpretation | 候选的 axis/series/range、参数映射是否完整且合法；这里的 projection 是 candidate 对 content matrix 的映射，不是 Table UI projection，也还不是最终 Template。 |
| `semanticScore` | `ReviewContext.evidence`；当前 tabular adapter 读取 `structuredEvidence.semanticCandidates` / columnProfiles / units | 列角色、单位、类型、单调性等是否支持 candidate。 |
| `dataQualityScore` | `ReviewContext.evidence`；当前 tabular adapter 读取 `structuredEvidence.structure` / measurement blocks | 行数、列数、空值比例、numeric coverage、block 稳定性。 |
| `parseHealthScore` | structured evidence 携带的 parser diagnostics | 解析健康程度；只作为扣分项，不替代 parser diagnostics。 |
| `freshnessScore` | `contentHash` / `sourceVersion` / `evidenceFingerprint`，以及可选 `materializationVersion` | 确认 candidate 对应当前内容版本和 evidence 快照；materialization version 只用于结构化模型变化导致的失效。 |
...
`resource URI` 关联到当前 content target；多 sheet/tabular content 通过 URI target
上的 `sheetId` 子维度区分。
...
| review validity | `ReviewResult` | `contentHash/sourceVersion`、`evidenceFingerprint`、recipe/user-template fingerprint、review policy version；可选 `materializationVersion` 仅作为结构化模型变化的失效输入 | 任一输入变化即 stale，不能继续当 ready。 |
| reviewed template | `ReviewResult.reviewedTemplate` | 跟随 review result | 只有 `ready` 才有；Slice 只消费这里的 reviewed/manual template。 |
| content / evidence snapshot | source/model snapshot | `resource URI` + optional `sheetId` target | 不保存 review decision，不反写 recipe 高置信度语义。 |
...
命名的 owner 文件；目标语义要收敛到 Explorer decoration。
...
| 字段 | 来源 | 说明 |
| --- | --- | --- |
| `resource` / `sheetId?` | review association | 被评估的 URI content association；`sheetId` 只是结构化内容的子目标。 |
...
来补做语义判断。
Explorer rich hover 也只能读同一个 `ReviewSummary`；例如文件数据被 Review 判定出的
curve kind、family、role、置信度和说明，都应作为 `ReviewSummary` / `ReviewedTemplate`
metadata 的摘要暴露给 Explorer，而不是让 Explorer 订阅或读取 `ReviewEvidence`。
`ResourceLabels`
...
URI-backed `Review` / Explorer decoration 以 `resource URI` + optional
`sheetId` target 查询 `IReviewService`。Explorer hover 和 label decoration
都消费同一份 `ReviewSummary`。
...
| `services/review/common/reviewEvidence.ts` | 定义 URI/content 进入 Review 的 evidence shape；当前 tabular 派生字段应表达为 structured/matrix evidence，只是 content evidence 的一种来源，不暴露 Table UI projection。 | Recipe DSL、review result/policy、Explorer decoration presentation。 |
...
| `contrib/files/browser/views/explorerDecorations.ts` | 可选 helper；只有 decoration 规则变多或多个 Files browser surface 复用时再拆。负责把 Explorer entry + `ReviewSummary` 映射成 `IDecorationData`。 | review cache、review validity。 |
| `contrib/files/common/explorerModel.ts` | 保留 Explorer entry/tree/presentation 类型；不再承载 semantic badge 状态字段。 | review cache、review validity。 |
| `contrib/files/browser/explorerViewlet.ts` | 注册 `ExplorerDecorationsProvider` 到 `IDecorationsService`，向 Explorer viewer 传递 decoration resource、badge text 用 `IDecorationData`，以及 hover 用 `ReviewSummary`。 | review 计算、candidate building、scoring、直接调用 provider。 |
...
| 类型 | 来源 | 语义 |
| --- | --- | --- |
| parser diagnostics | structured content parser / resolved evidence diagnostics | 文件是否能被健康规范化成结构化内容。 |
| review findings | `services/review/**` | 某个 `ReviewCandidate` 套在这个 URI 内容 / 结构化矩阵上是否可靠。 |

文件落点也要分清楚。Recipe 本身只是 catalog / physical-layout / logical-relation 定义；
把 Recipe 应用到 URI content evidence、产出 `ReviewCandidate` 的行为属于 review candidate building。
...
| 文件 | 职责 | 不拥有 |
| --- | --- | --- |
| `services/review/common/review.ts` | `IReviewService` 契约、URI review/manual review request/result 类型、review/evidence/result signature helper。 | evidence shape 定义、browser cache 实现、Explorer decoration 映射、Slice 提交。 |
| `services/review/common/reviewModel.ts` | Review 领域结果模型：`ReviewContext`、`SegmentCandidate`、`ReviewCandidate`、`ReviewResult`、`ReviewDecision`、factors、findings、`ReviewedTemplate`、`ReviewSummary`。 | service implementation、evidence 生产、Recipe catalog storage、Explorer UI 类型。 |
| `services/review/common/reviewEvidence.ts` | 暂定 Review 私有输入 evidence shape：source metadata 加 `structuredEvidence` / `matrixEvidence` 这类 content facts。当前 tabular 派生字段只能作为 structured/matrix evidence 的一种形态。 | `tableProjection` 目标 API、Table UI/model imports、Recipe DSL、Review policy/result assembly、Explorer presentation。 |
| `services/review/common/reviewSelector.ts` | Recipe dataRange / blockPartition / physicalLayout / logicalRelation 对 URI/content evidence 的纯匹配。 | 文件读取、parser 逻辑、candidate scoring、Template materialization。 |
| `services/review/common/reviewCandidate.ts` | Recipe/UserTemplate/built-in template snapshot + URI/content evidence -> `SegmentCandidate` / `ReviewCandidate`。 | 最终 Review status、`ReviewedTemplate` selection、Slice execution、Explorer decoration。 |
| `services/review/common/reviewScoring.ts` | candidate + `ReviewContext` -> confidence factors、findings 和 candidate status。 | candidate derivation、result cache、Explorer presentation、Slice policy。 |
| `services/review/common/reviewResult.ts` | 组合 context、candidate、scoring 和 decision policy，产出 `ReviewResult`、selected `ReviewedTemplate` 和 summary-ready facts。 | browser scheduling/cache、file/model reads、Explorer decoration mapping。 |
| `services/review/browser/reviewService.ts` | service owner：cache、stale、调度/后台 review、manual review 入口，以及从当前 source owner 读取/组装 structured content evidence。 | Table UI parsing、table projection ownership、DOM/UI decoration、Explorer tree state、Slice execution。 |

这些 `common/review*.ts` 文件是 Review 流程的纯 helper 拆分，不是多个服务。
`browser/reviewService.ts` 仍是完整 Review workflow 的 service owner：它负责调度、
cache、失效、manual review 入口，并编排 selector / candidate / scoring / result
这些纯阶段。

`ReviewEvidence` 暂定只给 Review 使用，所以可以先放在 `services/review/common`。
如果后续 Table UI、Review、Search、Vector 等多个 feature 都要复用同一套
structured/matrix evidence，再上移到中性的 content/table evidence owner，Review
只消费共享 snapshot。

| 目标层 | 上游对应 | 目标 owner / 文件 | 职责 | 不拥有 |
| --- | --- | --- | --- | --- |
| Recipe catalog | 无同名上游文件；机制上类似 language feature provider 的静态来源。 | `services/recipes/common/recipe.ts`、`recipeSchema.ts`、`recipeCodec.ts` | 定义 passive `Recipe`、dataRange / blockPartition / physicalLayout / logicalRelation authoring schema、校验、diagnostics、fingerprint。 | 不读取 table，不执行 Recipe，不产出 Template。 |
| Built-in recipe bundle | 无同名上游文件；更接近内置 provider/extension contribution 的数据来源。 | `resources/recipes/v1/**/*.json`、`resources/recipes/v1/index.json`、`scripts/buildRecipeBundle.mjs`、`services/recipes/common/builtinRecipes.generated.ts` | 手写 JSON recipe 和生成内置 bundle。 | 不放运行时 table/materialization 逻辑。 |
| Recipe service | `editor/common/services/languageFeatures.ts`、`editor/common/services/languageFeaturesService.ts`。 | `services/recipes/browser/recipeService.ts` | 发布 `RecipeSnapshot`、catalog fingerprint、diagnostics、`onDidChangeRecipes`。 | 不 evaluate tables，不构造 ReviewCandidate，不写 Session。 |
| Review types / context | `editor/common/model.ts`、`editor/common/model/textModel.ts` 作为 language feature 的输入模型。 | `services/review/common/reviewModel.ts`；URI/content evidence 放 `services/review/common/reviewEvidence.ts`。 | 定义 `ReviewContext`、`ReviewCandidate`、`ReviewResult`、`ReviewedTemplate`、findings/factors/decision 等纯类型；tabular 派生字段只作为 structured/matrix evidence 的一种形态。 | 不放 service implementation，不放 Session migration-ledger raw-table record。 |
...
| Review service implementation | 上游类似 browser/workbench service implementation。 | `services/review/browser/reviewService.ts`。 | 按 `resource URI` + optional `sheetId` target cache `ReviewResult`，处理 stale、订阅、后台 review，并编排 `reviewSelector` / `reviewCandidate` / `reviewScoring` / `reviewResult` 这些 common helper。内部可用 `ResourceMap` 或私有索引，但 API 名称保持 resource/target 口径。 | 不读取原始文件，不拥有 Table UI/model 内容，不把 candidate 作为独立 service/model。 |
| Explorer decorations service/provider | 上游 `decorationsService.ts`、`IDecorationsService`、`IDecorationsProvider`、`explorerDecorationsProvider.ts`。 | `services/decorations/**` + `contrib/files/browser/views/explorerDecorationsProvider.ts`，必要时配套同目录 `explorerDecorations.ts`。 | service 负责 provider 注册、缓存和 change event；Explorer provider 从 `IReviewService.getLatestReviewSummary(...)` 的 `ReviewSummary` 提供 `IDecorationData`、tooltip、color 和 presentation signature。 | 不 evaluate Recipe/Template。 |
| Review pipeline helpers | Review service 的纯流程阶段拆分；不是独立 service。 | `services/review/common/reviewSelector.ts`、`reviewCandidate.ts`、`reviewScoring.ts`、`reviewResult.ts`。 | selector matching、candidate building、scoring、result/summary assembly。 | browser cache、调度、file/model reads、Explorer presentation；不使用 `engine` 命名。 |
| Slice consumer | 无上游对应；Conductor 结构化内容分析域新增。 | `services/slice/**` | 执行 reviewed/manual Template snapshot。 | 不读取 Recipe，不推断 table semantics。 |

当前脚本落点跟随同一责任划分：`scripts/buildRecipeBundle.mjs` 生成
`services/recipes/common/builtinRecipes.generated.ts` 和
`cli/resources/recipes.v1.json`；`scripts/lib/review-process.mjs` 只为 bench /
compat 生成 Rust `processFile` 请求；Rust parity 脚本用显式 fixture 和 Rust
prepare seed 校验转换一致性。
...
在当前 tabular / structured adapter 迁移里，Slice 只接收已经通过 `Review` 的
...
其中 `resource URI` 指向被切片的 content target，多 sheet/tabular content 时
`sheetId` 只是 URI target 的结构化内容子维度；`reviewSignature` /
`templateFingerprint` 用来证明 Slice 使用的是哪一次 review 后的模板。`content_hash`
或 source version 用来证明同一 URI 下使用的是哪一版内容。文件内容、
structured evidence、可选 materialization snapshot、review result 任一变化，都应让旧 slice 结果失效或需要重新生成。URI-backed
slice 结果是 Slice service 的派生状态，不写入 Session migration-ledger raw-table pipeline。
...
| Slice request | `resource`、`contentHash/sourceVersion`、`sheetId?`、`ReviewedTemplate`、review signature、optional materialization version。 | 一个可执行的 URI slice job。 | Recipe catalog、selector matching、review scoring。 |
| Slice execution | `ReviewedTemplate` 定义的 ranges / axes / series / params。 | `SliceUriResult`。 | 修改 content/evidence snapshot、修改 `ReviewResult`、写 Session。 |
| Slice association | `resource URI` + content version + optional `sheetId` target + `reviewSignature/templateFingerprint`。 | 可查询的 slice 状态和结果；公开 API 用 resource/target 命名，内部索引不外露。 | Session migration-ledger raw-table identity、公开 synthetic key。 |

目标文件落点：

| 目标文件 | 职责 |
| --- | --- |
| `services/slice/common/slice.ts` | 定义 `ISliceService`、migration-ledger raw-table request、URI-backed target/request/result、slice signature 等纯类型。目标 API 应像上游 `createModelReference(resource)` 一样接收 `resource` / `target`，例如 submit target request、query latest result/state、订阅 target changes；不要暴露 keyed map 字段。 |
| `services/slice/browser/sliceService.ts` | 按 `resource URI` + optional `sheetId` target 关联 URI slice 结果，处理执行、cache、stale、event；内部可用 `ResourceMap` 或私有索引；Session migration-ledger raw-table 结果仍走 Session commit。 |
...
Table UI/model snapshot 核心内容。通用输出仍以 accepted `Segment` 为边界；
Slice 只是当前结构化内容分析域的执行型输出。

迁移参考：当前 tabular evidence 类型如果仍收敛在
`services/table/common/tableProjection.ts`，只能视为迁移期命名残留。按本文目标架构，
这些字段对应的是 URI-backed content matrix 之后的 `ReviewContext.evidence` /
content feature owner，不是 Recipe owner，也不是 Table UI projection owner。
首版可以先把这些字段作为 `ReviewContext` 的 structured evidence snapshot，不必单独服务化；
...
| `services/recipes/**` | Recipe 只保存 passive catalog，不 evaluate table。 |
...
| `reviewModel.ts` | 建议只放纯类型。 | 定义 `ReviewContext`、`ReviewCandidate`、`ReviewResult`、`ReviewedTemplate`、findings/factors/decision；URI/content evidence 放 `reviewEvidence.ts`；不要混入 service implementation 或 Session migration-ledger raw-table record。 |
| `reviewSelector.ts` / `reviewCandidate.ts` / `reviewScoring.ts` / `reviewResult.ts` | 保留为 Review 流程的纯 helper 拆分。 | `browser/reviewService.ts` 编排完整 Review workflow；这些 common helper 只承载 selector matching、candidate building、scoring 和 result assembly，不形成独立 service；不要使用 `engine` 命名。 |
...
`open -> render` 同步主路径；表格 UI 可以先打开和渲染，Review 再基于
contentHash/sourceVersion、evidenceFingerprint、可选 materializationVersion、
用户动作或后台任务执行。
...
| compat preview health summary | 只能是 `TableParseDiagnostic[]` 的 UI summary，不能成为 diagnostics owner。 |
...
旧的兼容 key 不是目标身份。只有还没有分配 `resource` 的 import staging，以及非 URI-backed 的 Session migration-ledger raw-table pipeline，才需要继续保留自己的临时/raw key。
...
| Session migration-ledger raw-table | raw/session identity | 需要保留 |
...
| 字段 | 当前定位 | 目标定位 |
| --- | --- | --- |
| 兼容 key | 历史上被 Explorer、view cache、列宽、migration-ledger raw pipeline 复用。 | URI-backed model 核心契约退场；只允许 migration-ledger raw 或 adapter 内部用 derived `TableSheetKey`。 |
...
| --- | --- | --- | --- |
| `TableModel(resource, compatKey)` | 把兼容 key 放进模型身份。 | `TableModel(resource)`，sheet 由 `defaultSheetId` / active sheet 表达。 | URI-backed 模型身份回到上游式 `resource URI`。 |
| `TableModelSheetSnapshot.compatKey` | 把 `resource URI + sheetId` 派生 key 存成 sheet 核心字段。 | `sheetId` / `sheetName` / `content` / `diagnostics`。 | 需要 key 时由 `toTableSheetKey(resource, sheetId)` 派生。 |
...
代码现实状态：URI-backed tableFile/table model 生产路径不应再暴露或依赖历史兼容 key 或预览投影输入。兼容期投影只能留在 adapter/migration-ledger 边界；URI-backed tableFile/table model 的目标契约不依赖它们。
...
| 无直接上游对应 | temporary import key / migration-ledger raw identity | 只保留在 pre-resource import staging 或 Session/raw-table；URI-backed 目标模型不以它为身份。 |
