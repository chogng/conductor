...
- `src/features/device-analysis/data/importWorkerClient.ts` 优先走 Rust Excel 转换。
...
- `src/features/device-analysis/data/usePreview.ts`
...
- 文件选择、队列调度、模板 UI、预览表格 UI、图表 UI、设置、session/cache policy 仍在 TypeScript/React。
- Rust 只返回数据结构，不负责 React 状态、交互、canvas 绘图。
...
- 不依赖 React 状态、DOM、canvas、Electron API 或 Origin COM。
